...
export default function KrEtfTaxMatrix({
  ...
  const stockRows = krStocks.map(stock => {
    const { purchases, sales, exTaxBase } = getCodeTaxBase(portfolio, stock.code);
    const currentQty = cleanNum(stock.quantity || 0);
    const dividendHistCode = portfolio?.dividendHistory?.[stock.code] || {};
    const appliedMap = portfolio?.dividendTaxAmounts?.[stock.code] || {};
    const monthData = monthYms.map(ym => {
      const hasDividend = ym in dividendHistCode && dividendHistCode[ym] > 0;
      const exVal = exTaxBase[ym];
      const result = hasDividend ? computeCodeMonthTax(portfolio, stock.code, ym, taxRate) : null;
      const applied = appliedMap[ym] || 0;
      return { ym, hasDividend, exVal, result, applied };
    });
    const annualComputed = monthData.reduce((s, d) => s + (d.result && !d.result.error ? d.result.tax : 0), 0);
    const annualApplied = monthData.reduce((s, d) => s + (d.applied || 0), 0);
    return { stock, purchases, sales, currentQty, monthData, annualComputed, annualApplied };
  });

  const monthlyTotals = monthYms.map((_, i) =>
    stockRows.reduce((s, r) => s + (r.monthData[i].result && !r.monthData[i].result.error ? r.monthData[i].result.tax : 0), 0),
  );
  const monthlyApplied = monthYms.map((_, i) =>
    stockRows.reduce((s, r) => s + (r.monthData[i].applied || 0), 0),
  );
  const grandComputed = monthlyTotals.reduce((s, v) => s + v, 0);
  const grandApplied = monthlyApplied.reduce((s, v) => s + v, 0);
  ...
  const applyMonthTax = (code, ym, result) => {
    if (!result || result.error) return;
    updatePortfolioDividendTaxAmount(portfolio.id, code, ym, result.tax);
    notify?.(`${ym} ${code} 세금 ${formatCurrency(result.tax)} 적용`, 'success');
  };
  const clearMonthTax = (code, ym) => {
    updatePortfolioDividendTaxAmount(portfolio.id, code, ym, 0);
    notify?.(`${ym} ${code} 적용 해제`, 'info');
  };

  return (
    <div className="overflow-x-auto">
      <div className="px-3 py-2 bg-[#0f172a]/60 border-b border-gray-700/50 flex items-center gap-3 flex-wrap text-[10px]">
        <span className="text-gray-500">평균법 · 세율 {taxRate}% · {CURRENT_YEAR}년</span>
        <span className="text-gray-600">|</span>
        <span className="text-gray-500">연간 산출 세금</span>
        <span className="text-orange-300 font-semibold tabular-nums">{formatCurrency(grandComputed)}</span>
        {grandApplied > 0 && <>
          <span className="text-gray-600">|</span>
          <span className="text-gray-500">표 적용 합계</span>
          <span className="text-emerald-400 font-semibold tabular-nums">{formatCurrency(grandApplied)}</span>
        </>}
      ...
      <table className="w-full text-[11px] text-center">
        <thead className="bg-[#1e293b] text-gray-400 border-b border-gray-600">
          <tr>
            ...
            {MONTHS.map(m => (
              <th key={m} className="py-2 px-1 min-w-[112px] font-normal">{m}</th>
            ))}
            <th className="py-2 px-2 min-w-[96px] text-yellow-500 font-bold">연합계</th>
          </tr>
        </thead>
        <tbody>
          {stockRows.map(({ stock, purchases, sales, currentQty, monthData, annualComputed, annualApplied }) => {
                ...
                <tr className="border-b border-gray-700/40 hover:bg-gray-800/20">
                  ...
                  {monthData.map(d => (
                    <td key={d.ym} className="py-1 px-1 align-top">
                      {!d.hasDividend ? (
                        <div className="text-[10px] text-gray-700 py-2">—</div>
                      ) : (
                        <div className="space-y-0.5">
                          ...
                          <div className="flex items-center justify-between gap-0.5 px-0.5">
                            {d.result?.error ? (
                              <span className="text-[9px] text-red-400 truncate flex-1" title={d.result.error}>⚠ 오류</span>
                            ) : d.result && d.result.totalShares > 0 ? (
                              <span className="text-[10px] text-orange-300 tabular-nums truncate flex-1" title={`세전 ${formatCurrency(d.result.grossDividend)} · 세후 ${formatCurrency(d.result.netDividend)}`}>
                                {formatCurrency(d.result.tax)}
                              </span>
                            ) : safeNum(d.exVal) > 0 ? (
                              <span className="text-[9px] text-gray-600 flex-1">매입 필요</span>
                            ) : (
                              <span className="text-[9px] text-gray-700 flex-1">—</span>
                            )}
                            {d.result && !d.result.error && d.result.totalShares > 0 && (
                              d.applied > 0 && Math.abs(d.applied - d.result.tax) < 1 ? (
                                <button
                                  onClick={() => clearMonthTax(stock.code, d.ym)}
                                  className="text-[9px] text-emerald-400 hover:text-emerald-300 px-1 rounded shrink-0"
                                  title="적용 해제"
                                >✓</button>
                              ) : (
                                <button
                                  onClick={() => applyMonthTax(stock.code, d.ym, d.result)}
                                  className="text-[9px] text-amber-400/70 hover:text-amber-300 hover:bg-amber-900/30 px-1 rounded shrink-0"
                                  title="이 월 세금을 분배금 표에 적용"
                                >✓</button>
                              )
                            )}
                          </div>
                          {d.applied > 0 && (!d.result || Math.abs(d.applied - (d.result.tax || 0)) >= 1) && (
                            <div className="text-[9px] text-emerald-500/70 tabular-nums" title="현재 분배금 표에 적용된 세금">
                              표 {formatCurrency(d.applied)}
                            </div>
                          )}
                        </div>
                      )}
                  ...
                  <td className="py-1 px-2 text-center tabular-nums">
                    <div className="text-[11px] text-orange-300 font-semibold">{annualComputed > 0 ? formatCurrency(annualComputed) : '-'}</div>
                    {annualApplied > 0 && (
                      <div className="text-[9px] text-emerald-500/70">표 {formatCurrency(annualApplied)}</div>
                    )}
                  </td>
                </tr>
                {isExpanded && (
          ...
          <tr>
            ...
            {monthlyTotals.map((v, i) => (
              <td key={i} className="py-1 px-1 tabular-nums">
                <div className="text-[10px] text-orange-300 font-semibold">{v > 0 ? formatCurrency(v) : '-'}</div>
                {monthlyApplied[i] > 0 && (
                  <div className="text-[9px] text-emerald-500/70">표 {formatCurrency(monthlyApplied[i])}</div>
                )}
              </td>
            ))}
            <td className="py-1 px-2 tabular-nums">
              <div className="text-[11px] text-yellow-400 font-bold">{grandComputed > 0 ? formatCurrency(grandComputed) : '-'}</div>
              {grandApplied > 0 && (
                <div className="text-[9px] text-emerald-500/70">표 {formatCurrency(grandApplied)}</div>
              )}
            </td>
          </tr>
        </tfoot>
      ...
      <div className="px-3 py-1.5 bg-[#0f172a]/60 text-[10px] text-gray-600 border-t border-gray-700/50">
        과표 입력 → 자동 계산 · <span className="text-amber-400/70">✓</span> = 적용 가능 · <span className="text-emerald-400">✓</span> = 분배금 표에 적용됨(클릭 시 해제) · 데이터는 자동 저장
      </div>
    </div>
  );
}

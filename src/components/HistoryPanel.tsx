...
입출금이 있던 날은 그 금액을 빼므로, 입금해도 수익으로 잡히지 않습니다.`}>{noun.unit} 수익</th>
                ...
                <tbody>
                  {viewRows.map((h, i) => {
                    ...
                    // ⚠️ 판정에 쓰인 값과 **같은 것**을 보여줘야 한다 — 흡수 판정은 `bookDelta − memoNet`
                    //    (미반영 원장 행의 순액을 걷어낸 값)을 쓰므로, raw를 띄우면서 '정확히 0이면
                    //    열립니다'라고 하면 미반영 행이 있는 날에 '+₩0인데 -'라는 자기모순이 화면에 뜬다.
                    ...
                    const heldBook = (!isPeriodMode && hasPrev && rowBookByDate && viewRows[i + 1] && m.holdReason !== 'no-data')
                      ? (() => {
                          ...
                          const memo = externalFlowInRange(depositHistory, depositHistory2, viewRows[i + 1].date, h.date).memoNet || 0;
                          return raw - memo;
                        })() : null;
                    const heldBookLine = heldBook == null ? ''
                      : `\n장부액(예수금+매입원가, 미반영 기록 제외) 전일 대비 ${heldBook >= 0 ? '+' : '−'}${formatCurrency(Math.abs(heldBook))} — 원장 흐름과 맞지 않아 보류했습니다(정확히 0이면 표시가 열립니다).`;

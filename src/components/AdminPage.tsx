...
export default function AdminPage({ adminEmail, onClose, onViewUser, onOpenPortal, userAccessStatus = {}, switching = false, userLastSeen = {}, userDriveStatus = {}, onRefreshUserSessions, youtubeUrl = '', onSetYoutubeUrl, notebookLinks = [], onSetNotebookLinks, reportUrl = '', onSetReportUrl, noticeFlags = { notebook: true }, onSetNoticeFlags, onUploadStudyMaterial, onDeleteStudyMaterialFile }: Props) {
  ...
  // 기능 설정 상태
  const [featureLabels, setFeatureLabels] = useState(['기능1', '기능2', '기능3', '유튜브', '학습자료', '시장리포트', '자금흐름도', '백테스트', '가계부']);
            ...
            <ul className="space-y-2 max-h-72 overflow-y-auto">
              {users.map((u, i) => {
                ...
                const featureDefs = [
                  ...
                  { feat: 'ledgerEnabled',  label: featureLabels[8], val: !!u.ledgerEnabled,  onCls: 'text-amber-300 border-amber-700/60 bg-amber-900/40',    offCls: 'text-gray-600 border-gray-700/30 bg-gray-800/40' },
                ];
                return (
                  <li key={i} className="bg-gray-800 rounded-lg px-3 py-2">
                    ...
                    {/* 기능 ON/OFF 토글 — 비관리자 전용.
                        ...
                        ⚠️ 이 주석을 아래 여는 괄호 안쪽으로 옮기지 말 것 — 거기는 표현식 위치라
                        JSX 주석이 아니라 빈 객체 리터럴로 파싱돼 빌드가 깨진다. */}
                    {!isAdminUser && (

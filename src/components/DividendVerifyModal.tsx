...
// 하단: 월별 합계. 분배락 일자·지급예정일 컬럼은 제외(요구사항).
...
export default function DividendVerifyModal({ portfolios, dividendTaxHistory = {}, usdkrw = 1300, onClose }) {
  ...
  const { months, rows, columnTotals, grandTotal, grandTaxable, taxableCovered } = useMemo(() => {
    ...
    const colTotals = monthList.map((_, i) =>
      rowList.reduce((s, r) => s + (r.cells[i] || 0), 0)
    );
  ...
  }, [portfolios, dividendTaxHistory, usdkrw, year]);
                ...
                <tbody>
                  ...
                  {rows.map(r => (
                    <tr key={r.key} className="border-t border-gray-700/60 hover:bg-gray-800/40">
                      ...
                      {r.cells.map((c, i) => (
                        <td key={i} className="py-1.5 px-2 text-gray-300 border-r border-gray-700/30">
                          {c != null ? formatCurrency(c) : <span className="text-gray-600">-</span>}
                        </td>
                      ))}

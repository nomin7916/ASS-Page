...
export default function PortfolioChart({
  ...
  // 수익률 라인 = **항상 누적 TWR 재베이스**('기간 수익률', 2026-07 통일 — 시계 아이콘 토글 제거).
  //   입출금 보정 완료 → 입금해도 곡선이 왜곡되지 않는다. 원금대비 (V−C)/C는 요약 카드·추이 표
  //   누적 열에만 남는다(차트 라인 아님).
  // ⚠️ 라인을 'V(t)÷V(시작)−1'(평가액 증감률: 입금액이 분자에 들어가 +747%로 부풀고 실손익 −536만)나
  //    원금대비(입금일 분모 급증 절벽)로 되돌리지 말 것 — 라벨도 '평가액 증감'으로 되돌리지 말 것.
  // ⚠️ 범례·툴팁·정보패널이 반드시 같은 라벨을 써야 한다. chartUtils의 [구간: x%] 태그 매칭도
  //    이 문자열을 보므로, 여기를 바꾸면 chartUtils의 name 비교도 같이 바꿀 것.
  const myReturnLabel = '기간 수익률';
        ...
        <div className="flex justify-between items-center pt-2 border-t border-gray-700/50">
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              title="기간 수익률(%) 표시 — 조회시작을 0%로 두고 입출금 영향을 제거한 순수 시장 수익률(TWR)입니다. 입금·출금이 있어도 곡선이 왜곡되지 않습니다."
                          ...
                          <>
                            <div className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-0.5 text-[10px] pl-3.5">
                              ...
                              <span className="text-gray-400 leading-snug">
                                {'일간 수익률 = 자산 평가액 추이 표의 일간 수익률과 동일한 입출금 보정값(Modified Dietz — 입금은 기초·출금은 기말 가중) · 평가자산 = 당일 총평가액(예수금 포함, 수량×종가 재계산) · 주말·휴장 등 평가 변동이 없는 날은 배율 1(직전값 유지)'}
                              </span>
                            </div>
                            <div className="mt-1.5 ml-3.5 p-1.5 rounded bg-gray-800/40 border border-gray-700/40 text-[9.5px] text-gray-400 leading-relaxed">
                              <>ℹ️ 조회 시작일을 0%로 둔 <b>기간 수익률(TWR)</b>입니다. 입금·출금은 일간 수익률 단계에서 이미 제거되므로 <b className="text-green-400">입출금 규모가 곡선을 왜곡하지 않습니다</b> — 돈을 넣었다고 수익률이 오르거나, 원금이 늘어 수익률이 희석되는 일이 없습니다. 지수·비교종목 선과 같은 기준(시작 0%)이라 직접 비교할 수 있습니다. 내 돈이 원금 대비 얼마나 불었는지는 요약 카드·자산 평가액 추이 표에서 확인할 수 있습니다.</>
                            </div>
                          </>
                        ) : (
      ...
      >
        <ResponsiveContainer width="100%" height="100%" minHeight={260}>
          <ComposedChart data={finalChartData} onMouseDown={handleChartMouseDown} onMouseMove={handleChartMouseMove} onMouseUp={handleChartMouseUp} onMouseLeave={handleChartMouseLeave}>
            ...
            {showTotalEval && <Area isAnimationActive={false} yAxisId="right" type="monotone" dataKey="evalAmount" name="총자산" fill="rgba(156, 163, 175, 0.1)" stroke="#9ca3af" strokeWidth={2} dot={false} activeDot={{ r: 5 }} />}
            {showPrincipal && <Line isAnimationActive={false} yAxisId="right" type="monotone" dataKey="principalAmount" name="투자원금" stroke="#22d3ee" strokeWidth={1.5} dot={false} strokeDasharray="5 3" connectNulls />}
            {showReturnRate && <Area isAnimationActive={false} yAxisId="left" type="monotone" dataKey="principalReturnRate" name={myReturnLabel} fill="rgba(239, 68, 68, 0.1)" stroke="#ef4444" strokeWidth={2} dot={false} activeDot={{ r: 5 }} connectNulls />}
            {userFeatures.feature1 && showKospi && <Line isAnimationActive={false} yAxisId="left" type="monotone" dataKey="kospiRate" name="KOSPI" stroke="#38bdf8" strokeWidth={1.5} dot={false} strokeDasharray="3 3" filter="url(#neonGlow)" />}
            {userFeatures.feature1 && showSp500 && <Line isAnimationActive={false} yAxisId="left" type="monotone" dataKey="sp500Rate" name="S&P500" stroke="#bf5af2" strokeWidth={1.5} dot={false} strokeDasharray="3 3" filter="url(#neonGlow)" />}
            {userFeatures.feature1 && showNasdaq && <Line isAnimationActive={false} yAxisId="left" type="monotone" dataKey="nasdaqRate" name="NASDAQ" stroke="#30d158" strokeWidth={1.5} dot={false} strokeDasharray="3 3" filter="url(#neonGlow)" />}
            {userFeatures.feature1 && effectiveShowIndicators.us10y && indicatorHistoryMap.us10y && <Line isAnimationActive={false} yAxisId="left" type="monotone" dataKey="us10yRateScaled" name="US 10Y" stroke="#8e8e93" strokeWidth={1.5} dot={false} strokeDasharray="4 2" connectNulls />}
            {isXl && userFeatures.feature1 && effectiveShowIndicators.us10y && indicatorHistoryMap.us10y && <Line isAnimationActive={false} yAxisId="right-us10y" dataKey="us10yPoint" stroke="transparent" dot={false} activeDot={false} legendType="none" tooltipType="none" connectNulls />}
            {userFeatures.feature1 && effectiveShowIndicators.goldIntl && indicatorHistoryMap.goldIntl && <Line isAnimationActive={false} yAxisId="left" type="monotone" dataKey="goldIntlRateScaled" name="Gold" stroke={goldIndicatorColors.goldIntl} strokeWidth={1.5} dot={false} strokeDasharray={activePortfolioAccountType === 'gold' ? undefined : "4 2"} connectNulls />}
            {isXl && userFeatures.feature1 && effectiveShowIndicators.goldIntl && indicatorHistoryMap.goldIntl && <Line isAnimationActive={false} yAxisId="right-goldIntl" dataKey="goldIntlPoint" stroke="transparent" dot={false} activeDot={false} legendType="none" tooltipType="none" connectNulls />}
            {userFeatures.feature1 && effectiveShowIndicators.goldKr && indicatorHistoryMap.goldKr && (
              activePortfolioAccountType === 'gold'
                ? <Area isAnimationActive={false} yAxisId="left" type="monotone" dataKey="goldKrRateScaled" name="국내금" stroke={goldIndicatorColors.goldKr} strokeWidth={2} fill="url(#goldKrGradient)" dot={false} connectNulls />
                : <Line isAnimationActive={false} yAxisId="left" type="monotone" dataKey="goldKrRateScaled" name="국내금" stroke={goldIndicatorColors.goldKr} strokeWidth={1.5} dot={false} strokeDasharray="4 2" connectNulls />
            )}
            {isXl && userFeatures.feature1 && effectiveShowIndicators.goldKr && indicatorHistoryMap.goldKr && <Line isAnimationActive={false} yAxisId="right-goldKr" dataKey="goldKrPoint" stroke="transparent" dot={false} activeDot={false} legendType="none" tooltipType="none" connectNulls />}
            {userFeatures.feature1 && effectiveShowIndicators.usdkrw && indicatorHistoryMap.usdkrw && <Line isAnimationActive={false} yAxisId="left" type="monotone" dataKey="usdkrwRateScaled" name="USDKRW" stroke={goldIndicatorColors.usdkrw} strokeWidth={1.5} dot={false} strokeDasharray={activePortfolioAccountType === 'gold' ? undefined : "4 2"} connectNulls />}
            {isXl && userFeatures.feature1 && effectiveShowIndicators.usdkrw && indicatorHistoryMap.usdkrw && <Line isAnimationActive={false} yAxisId="right-usdkrw" dataKey="usdkrwPoint" stroke="transparent" dot={false} activeDot={false} legendType="none" tooltipType="none" connectNulls />}
            {userFeatures.feature1 && effectiveShowIndicators.dxy && indicatorHistoryMap.dxy && <Line isAnimationActive={false} yAxisId="left" type="monotone" dataKey="dxyRateScaled" name="DXY" stroke={goldIndicatorColors.dxy} strokeWidth={1.5} dot={false} strokeDasharray={activePortfolioAccountType === 'gold' ? undefined : "4 2"} connectNulls />}
            {isXl && userFeatures.feature1 && effectiveShowIndicators.dxy && indicatorHistoryMap.dxy && <Line isAnimationActive={false} yAxisId="right-dxy" dataKey="dxyPoint" stroke="transparent" dot={false} activeDot={false} legendType="none" tooltipType="none" connectNulls />}
            {userFeatures.feature1 && effectiveShowIndicators.fedRate && indicatorHistoryMap.fedRate && <Line isAnimationActive={false} yAxisId="left" type="monotone" dataKey="fedRateRateScaled" name="기준금리" stroke="#ff375f" strokeWidth={1.5} dot={false} strokeDasharray="4 2" connectNulls />}
            {isXl && userFeatures.feature1 && effectiveShowIndicators.fedRate && indicatorHistoryMap.fedRate && <Line isAnimationActive={false} yAxisId="right-fedRate" dataKey="fedRatePoint" stroke="transparent" dot={false} activeDot={false} legendType="none" tooltipType="none" connectNulls />}
            {userFeatures.feature1 && effectiveShowIndicators.kr10y && indicatorHistoryMap.kr10y && <Line isAnimationActive={false} yAxisId="left" type="monotone" dataKey="kr10yRateScaled" name="KR 10Y" stroke="#636366" strokeWidth={1.5} dot={false} strokeDasharray="4 2" connectNulls />}
            {isXl && userFeatures.feature1 && effectiveShowIndicators.kr10y && indicatorHistoryMap.kr10y && <Line isAnimationActive={false} yAxisId="right-kr10y" dataKey="kr10yPoint" stroke="transparent" dot={false} activeDot={false} legendType="none" tooltipType="none" connectNulls />}
            {userFeatures.feature1 && effectiveShowIndicators.vix && indicatorHistoryMap.vix && <Area isAnimationActive={false} yAxisId="left" type="monotone" dataKey="vixRateScaled" name="VIX" stroke="#ff453a" strokeWidth={1.5} fill="url(#vixGradient)" strokeDasharray="4 2" connectNulls dot={false} />}
            {isXl && userFeatures.feature1 && effectiveShowIndicators.vix && indicatorHistoryMap.vix && <Line isAnimationActive={false} yAxisId="right-vix" dataKey="vixPoint" stroke="transparent" dot={false} activeDot={false} legendType="none" tooltipType="none" connectNulls />}
            {userFeatures.feature1 && effectiveShowIndicators.btc && indicatorHistoryMap.btc && <Line isAnimationActive={false} yAxisId="left" type="monotone" dataKey="btcRateScaled" name="Bitcoin" stroke="#f7931a" strokeWidth={1.5} dot={false} strokeDasharray="4 2" connectNulls />}
            {isXl && userFeatures.feature1 && effectiveShowIndicators.btc && indicatorHistoryMap.btc && <Line isAnimationActive={false} yAxisId="right-btc" dataKey="btcPoint" stroke="transparent" dot={false} activeDot={false} legendType="none" tooltipType="none" connectNulls />}
            {userFeatures.feature1 && effectiveShowIndicators.eth && indicatorHistoryMap.eth && <Line isAnimationActive={false} yAxisId="left" type="monotone" dataKey="ethRateScaled" name="Ethereum" stroke="#627eea" strokeWidth={1.5} dot={false} strokeDasharray="4 2" connectNulls />}
            {isXl && userFeatures.feature1 && effectiveShowIndicators.eth && indicatorHistoryMap.eth && <Line isAnimationActive={false} yAxisId="right-eth" dataKey="ethPoint" stroke="transparent" dot={false} activeDot={false} legendType="none" tooltipType="none" connectNulls />}
            ...
            {showBacktest && <Area isAnimationActive={false} yAxisId="left" type="monotone" dataKey="backtestRate" name="백테스트(현재비중)" stroke={backtestColor} strokeWidth={2} fill="url(#backtestGradient)" strokeDasharray="6 3" dot={false} connectNulls />}

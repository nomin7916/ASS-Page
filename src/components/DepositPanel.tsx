  ...
  return (
    <>
          ...
          {/* ⚠️ min-h를 빼지 말 것 (모바일에서 카드가 헤더만 남고 붕괴한다 — 사용자 보고 2026-09):
              ...
              해외 분기는 `min-h`가 있어 원래부터 무사했고 국내 분기만 무너졌다.
              PC(xl↑)는 main axis가 가로라 height가 그대로 살아나므로 이 min-h는 PC 레이아웃을 바꾸지 않는다.
              `flex-1`을 건드리면 PC의 가로 균등 분배가 달라지므로 그쪽은 손대지 않는다. */}
          <div className={`flex-1 w-full bg-[#1e293b] rounded-xl border border-gray-700 shadow-lg ${isOverseas ? 'h-full min-h-[552px]' : 'h-[392px] min-h-[392px]'} flex flex-col overflow-hidden`}>
            ...
            <div className="flex-1 overflow-y-auto">
              <table className="w-full text-right text-[11px] table-fixed">
                <thead className="sticky top-0 bg-gray-800 text-gray-400 border-b border-gray-600 shadow-sm z-10">
                  <tr>
                    <th className={`py-1.5 ${isOverseas ? 'w-[50px]' : 'w-[55px]'} text-center border-r border-gray-600 font-normal cursor-pointer hover:bg-gray-700`} onClick={() => handleDepositSort('date')}>일자{sortArrow(depositSortConfig, 'date')}</th>
                    <th className={`py-1.5 border-r border-gray-600 px-1 ${isOverseas ? 'w-[55px]' : 'w-[65px]'} text-blue-300 font-normal text-center cursor-pointer hover:bg-gray-700`} onClick={() => handleDepositSort('amount')}>{isOverseas ? '금액($)' : '금액'}{sortArrow(depositSortConfig, 'amount')}</th>
          ...
          <div className={`flex-1 w-full bg-[#1e293b] rounded-xl border border-gray-700 shadow-lg ${isOverseas ? 'h-full min-h-[552px]' : 'h-[392px] min-h-[392px]'} flex flex-col overflow-hidden`}>
            ...
            <div className="flex-1 overflow-y-auto">
              <table className="w-full text-right text-[11px] table-fixed">
                <thead className="sticky top-0 bg-gray-800 text-gray-400 border-b border-gray-600 shadow-sm z-10">
                  <tr>
                    <th className={`py-1.5 ${isOverseas ? 'w-[50px]' : 'w-[55px]'} text-center border-r border-gray-600 font-normal cursor-pointer hover:bg-gray-700`} onClick={() => handleDepositSort2('date')}>일자{sortArrow(depositSortConfig2, 'date')}</th>
                    <th className={`py-1.5 border-r border-gray-600 px-1 ${isOverseas ? 'w-[55px]' : 'w-[65px]'} text-blue-300 font-normal text-center cursor-pointer hover:bg-gray-700`} onClick={() => handleDepositSort2('amount')}>{isOverseas ? '금액($)' : '금액'}{sortArrow(depositSortConfig2, 'amount')}</th>
                    {isOverseas && <th className="py-1.5 border-r border-gray-600 px-1 w-[46px] text-sky-400 font-normal text-center">환율</th>}
                    <th className={`py-1.5 border-r border-gray-600 px-1 ${isOverseas ? 'w-[70px]' : 'w-[75px]'} text-yellow-400 font-normal text-center`}>{isOverseas ? '합계($)' : '합계'}</th>
                    <th className="py-1.5 border-r border-gray-600 text-center px-1 font-normal whitespace-nowrap">메모</th>
                    <th className="py-1.5 w-[28px] text-center font-normal">
            ...
            <div className="bg-black border-b border-gray-900 px-3 py-2 flex items-center justify-between cursor-move select-none" onMouseDown={handleMemoDragStart}>
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="w-[18px] h-[18px] rounded-full bg-pink-600 hover:bg-pink-400 flex items-center justify-center group transition-all"
                  title="취소 (Esc)"
                >
                ...
                <button
                  ...
                  className="w-[18px] h-[18px] rounded-full bg-purple-600 hover:bg-purple-400 flex items-center justify-center group transition-all"
                  title="저장 (Ctrl+Enter)"
                >

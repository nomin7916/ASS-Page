...
const UI_CONFIG = {
  VERSION: "v2.9.7 Professional (UI & Retry Logic)",
  COLORS: {
...
const formatCurrency = (n) => new Intl.NumberFormat('ko-KR', { style: 'currency', currency: 'KRW' }).format(cleanNum(n));
const formatPercent = (n) => cleanNum(n).toFixed(2) + '%';
const formatNumber = (n) => (n === '' || n == null) ? '' : new Intl.NumberFormat('ko-KR').format(cleanNum(n));
const formatChangeRate = (n) => {
  const s = cleanNum(n);
...
const getClosestValue = (dataObj, targetDateStr) => {
...
const handleTableKeyDown = (e, colKey) => {
  if (e.key === 'Enter') {
    e.preventDefault();
    e.target.blur();
...
const buildIndexStatus = (data, source) => {
...
const parseIndexCSV = (text, fileName) => {
  const lines = text.trim().split('\n');
...
const detectIndexFromFileName = (fileName) => {
...
export default function App() {
  ...
  const fetchMarketIndicators = async () => {
    ...
    // 최초 1회 전체 동시 호출
    const keys = Object.keys(fetchersMap);
    await Promise.all(keys.map(async (key) => {
      const res = await fetchersMap[key](now, statusMap);
      resultsMap[key] = res;
    }));
    ...
    // 실패한 키 추출
    const failedKeys = keys.filter(k => resultsMap[k].price === null);
    
    if (failedKeys.length > 0) {
       // 백그라운드 재시도 큐 진입
       retryFailedIndicators(failedKeys, statusMap, 10);
    } else {
       showToast(`시장 지표 10건 전체 수집 완료`);
    }
  };

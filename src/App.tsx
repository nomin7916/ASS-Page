            ...
            <div className="chart-container-for-drag p-4 h-[400px] relative select-none">
              ...
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={finalChartData} onMouseDown={handleChartMouseDown} onMouseMove={handleChartMouseMove} onMouseUp={handleChartMouseUp} onMouseLeave={handleChartMouseUp}>
                  ...
                  {showIndicatorsInChart.us10y && indicatorHistoryMap.us10y && <YAxis yAxisId="right-us10y" orientation="right" stroke="#8e8e93" tick={{ fontSize: 9 }} tickFormatter={v => Number(v).toFixed(2)} domain={(() => { const vs = finalChartData.map(d => d.us10yPoint).filter(v => v != null); if (!vs.length) return ['auto','auto']; const mn=Math.min(...vs),mx=Math.max(...vs),p=Math.abs(mx-mn)*0.05||0.1; return [mn-p,mx+p]; })()} />}
                  {showIndicatorsInChart.goldIntl && indicatorHistoryMap.goldIntl && <YAxis yAxisId="right-goldIntl" orientation="right" stroke="#ffd60a" tick={{ fontSize: 9 }} tickFormatter={v => Math.round(v).toLocaleString()} domain={(() => { const vs = finalChartData.map(d => d.goldIntlPoint).filter(v => v != null); if (!vs.length) return ['auto','auto']; const mn=Math.min(...vs),mx=Math.max(...vs),p=Math.abs(mx-mn)*0.05||1; return [mn-p,mx+p]; })()} />}
                  {showIndicatorsInChart.usdkrw && indicatorHistoryMap.usdkrw && <YAxis yAxisId="right-usdkrw" orientation="right" stroke="#0a84ff" tick={{ fontSize: 9 }} tickFormatter={v => Math.round(v).toLocaleString()} domain={(() => { const vs = finalChartData.map(d => d.usdkrwPoint).filter(v => v != null); if (!vs.length) return ['auto','auto']; const mn=Math.min(...vs),mx=Math.max(...vs),p=Math.abs(mx-mn)*0.05||1; return [mn-p,mx+p]; })()} />}
                  {showIndicatorsInChart.dxy && indicatorHistoryMap.dxy && <YAxis yAxisId="right-dxy" orientation="right" stroke="#5ac8fa" tick={{ fontSize: 9 }} tickFormatter={v => Number(v).toFixed(1)} domain={(() => { const vs = finalChartData.map(d => d.dxyPoint).filter(v => v != null); if (!vs.length) return ['auto','auto']; const mn=Math.min(...vs),mx=Math.max(...vs),p=Math.abs(mx-mn)*0.05||0.5; return [mn-p,mx+p]; })()} />}
                  {showIndicatorsInChart.fedRate && indicatorHistoryMap.fedRate && <YAxis yAxisId="right-fedRate" orientation="right" stroke="#ff375f" tick={{ fontSize: 9 }} tickFormatter={v => Number(v).toFixed(2)+'%'} domain={(() => { const vs = finalChartData.map(d => d.fedRatePoint).filter(v => v != null); if (!vs.length) return ['auto','auto']; const mn=Math.min(...vs),mx=Math.max(...vs),p=Math.abs(mx-mn)*0.05||0.1; return [mn-p,mx+p]; })()} />}
                  {showIndicatorsInChart.kr10y && indicatorHistoryMap.kr10y && <YAxis yAxisId="right-kr10y" orientation="right" stroke="#636366" tick={{ fontSize: 9 }} tickFormatter={v => Number(v).toFixed(2)+'%'} domain={(() => { const vs = finalChartData.map(d => d.kr10yPoint).filter(v => v != null); if (!vs.length) return ['auto','auto']; const mn=Math.min(...vs),mx=Math.max(...vs),p=Math.abs(mx-mn)*0.05||0.1; return [mn-p,mx+p]; })()} />}
                  {showIndicatorsInChart.vix && indicatorHistoryMap.vix && <YAxis yAxisId="right-vix" orientation="right" stroke="#ff453a" tick={{ fontSize: 9 }} tickFormatter={v => Number(v).toFixed(1)} domain={(() => { const vs = finalChartData.map(d => d.vixPoint).filter(v => v != null); if (!vs.length) return ['auto','auto']; const mn=Math.min(...vs),mx=Math.max(...vs),p=Math.abs(mx-mn)*0.05||1; return [mn-p,mx+p]; })()} />}

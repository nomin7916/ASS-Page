...
export const holdReasonText = (reason, opts?) => {
  ...
  switch (reason) {
    case 'unreflected-idle':
      // ⚠️ '그날의 시세 변동분'이라 단언하지 말 것 — 기준(anchor) 행 규약상 직전 행이 보류였으면 이 값은
      //    그 구간 전체의 변동이다. 구간 여부는 호출부가 `spanFromText`로 덧붙이므로 여기선 중립 표현을 쓴다.
      return [pending,
        '장부액(예수금+매입원가)이 전일과 같습니다 — 포트폴리오 표의 예수금이 아직 줄지/늘지 않았다는 뜻입니다.',
        `표시된 값은 입출금을 뺀 시세 변동분입니다. 그 날짜의 자산검증에서 예수금을 맞추면 정산됩니다.`].join('\n');
    case 'frozen-idle':
    ...
    case 'no-data':
      // ⚠️ '다음 산출 행에 합산됩니다'라고 쓰지 말 것 — `no-data`는 기준(anchor) 행을 **전진시키는**
      //    유일한 보류 사유라(computeDailyMetricsSeries), 이 행의 변동은 어디에도 합산되지 않는다.
      //    직전 평가액이 0이면 비율의 분모가 없어 애초에 비교가 성립하지 않기 때문이다.
      return '직전 기록의 평가액이 0이라 비교 기준이 없어 산출하지 않습니다 — 다음 기록부터 이 행을 기준으로 다시 계산합니다.';
    default:

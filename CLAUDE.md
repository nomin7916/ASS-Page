...
- **⚠️ 목표비중 슬롯이 투자선택별로 분리돼 있다 — `utils.resolveTargetSlots`가 유일한 결정 지점**:
  ...
  적립식은 '이번 적립금을 어떻게 나눌지'라 '자산 전체 목표 배분'과 의미가 달라서다. 그 결과
  **적립식으로 가면 자연히 0%에서 시작하고, 리밸런싱으로 돌아오면 쓰던 비중이 그대로 살아 있다**
  (사용자 요구 "적립식이면 0% 초기화 / 리밸런싱이면 사용자 비중"을 유실 없이 동시 충족).
  ...
  비중으로 추종시켜 '적립식은 0%에서 시작' 규약이 깨진다.
  ...
  **배포 직후 동작(의도)**: 기존 적립식 계좌는 첫 로드에서 비중이 0%로 보인다 — 값은 리밸런싱 슬롯에
  그대로 있어 모드를 리밸런싱으로 바꾸면 즉시 복귀한다(유실 아님).
...
- **화면 레이아웃 규약 (2026-08 사용자 요청 — 되돌리지 말 것)**:
  ...
  - **⚠️ 팝오버 지연 닫기 4경로는 한 세트다**(`useHoverPop`, `POP_GRACE_MS` 140ms — 검증 #259g):
    ...
    `onMouseEnter` → `cancel` ③ 팝오버 `onMouseLeave` → `close`(**재예약** — 빠지면 z-1200 패널이
    영구 고착돼 아래 표의 클릭을 삼킨다) ④ scroll/resize·`onBlur`·`Hint` 클릭·언마운트는 `closeNow`(즉시).
...
- **기말 예수금 원천별 분해 (⚠️ 항등식 — 검증 #110)**:
  `기말 예수금 = 초기 매수 후 잔여 + 누적 매매차익 + 종목 재편 순현금 + 누적 분배금`.
  기말 보유 표와 CSV가 이 분해를 그대로 렌더하므로 깨지면 소계가 예수금과 안 맞는다.
...
푼다"는 운용을 재현하기 위한 `BtConfig` 6필드. **`dip`(시그널 리밸런싱)만 독립 트리거**이고 나머지
5종은 **정기 리밸런싱에만** 걸린다 — 초기 매수·이벤트(구조 변경)·분배금 재투자는 종전 규약 그대로다
(그쪽까지 번지면 PDF 재현 #23~#42가 깨진다).

| 필드 | 기본값 | 요약 |
|---|---|---|
| `band` | 0 | `\|리밸런싱 전 평가액 − 목표\| ≤ 목표 × band%` 면 그 회차 매매 생략 |
| `buyFunding` | `'both'` | `'tradeOnly'`면 평시 매수가 `cashTrade`만 쓴다(분배금 주머니 잠금) |
| `dip` | `{enabled:false}` | **시그널 리밸런싱** — 매수=가격 고점 대비 −N% / 매도=가격 저점 대비 +N%, **발동일 종가로 즉시 체결** |
...
- **⚠️ 매수 재원 3단계: 매매 주머니 → 재조정 → 분배금 개방**(사용자 정의 2026-08).
  ① `cashTrade`로 산다. ② 모자라면(`dip.reallocate`, 기본 true) **목표를 초과한 다른 보유 종목**을
  목표까지 팔아 재원을 만든다(초과분이 큰 종목부터, `t.signal='realloc'`). ③ 그래도 모자라면 분배금
  주머니를 연다. **전 종목이 함께 하락해 팔 초과분이 없으면** ②가 아무것도 하지 않고 ③으로 넘어가는데,
  그것이 "보유 종목 전체가 하락했고 예수금이 없으면 누적 분배금을 쓴다"는 사양의 경로다(가드 #233b).
...
- **⚠️ 개방 한도는 종목별**(그 종목 단계들의 합)이다 — 여러 종목이 같은 날 발동해도 서로의 몫을 빼앗지
  않아 화면의 **"사용 ≤ 개방"** 표시 불변식이 성립한다(옛 `dipUnlock`과 같은 규약, 미러 #250b).
  매수는 매매 주머니를 먼저 쓰므로 `signalEvents[].used`가 0일 수 있다(정상).
  ⚠️ 개방액은 **매수 계획이 살아 있는 종목만** 채운다 — 종가가 없어 계획에서 탈락한 종목까지 채우면
  화면에 '실제로 열린 적 없는 개방 계산식'이 렌더된다.
  ⚠️ 같은 종목의 여러 단계가 같은 날 겹치면(갭 하락) `unlocked`가 **합**이므로 비율도 합
  (**`unlockPctSum`**)을 함께 남긴다 — 단계별 `unlockPct`로 화면을 그리면 `₩1,000,000 × 34% = ₩670,000`
  같은 **거짓 계산식**이 찍힌다(적대적 리뷰 확정, 미러 #321 / 가드 #259b). 비-carrier 행은 두 값이 0이라
  화면이 개방 줄 자체를 렌더하지 않는다.
  ⚠️ **`buyFunding:'both'`는 '개방'이라는 개념 자체가 없다** — 평시에 이미 주머니가 열려 있다.
  `unlocked`를 **0**으로 기록하고 화면이 문장으로 설명한다. 옛 코드처럼 '주머니 전액'을 종목마다
  기록하면 같은 날 N종목이 발동했을 때 KPI 합계가 **존재한 적 없는 N배 금액**을 보고한다
  (적대적 리뷰 확정, 미러 #322). KPI '개방/사용' 줄도 `tradeOnly`일 때만 개방을 표시한다.
...
- **⚠️ `signalEvents`는 화면이 계산식을 재현할 수 있게 밑변까지 남긴다**(사용자 요청 2026-08:
  "'개방'이라는 표현을 `1단계 · 적립 분배금 ₩A × 34% = ₩B`처럼 상세히"). 필드: `kind`(buy/sell) ·
  `step`(1부터) · `level` · `unlockPct` · **`divPocketAt`(발동 시점 적립 분배금 = 계산식의 밑변)** ·
  `cashTradeAt` · `unlocked` · `used` · `ref`(고점 또는 저점) · `price` · `tradeQty` · `tradeAmount` ·
  `fromTrade` · `reallocAmount` · `note`. 옛 화면은 `개방 ₩0 → 사용 ₩0` 한 줄이라 **왜 0인지** 알 수
  없었다 — 미체결이면 `note`(재원 없음·목표 이상·종가 없음 등)를 그대로 보여 준다(가드 #259b).
  문장 포매터(`sigLabel`·`sigRefText`·`sigUnlockText`·`sigFundText`·`sigOutcomeText`)는 **화면과 CSV가
  공유**한다 — 각자 만들면 같은 사건이 화면과 파일에서 다르게 설명된다.
  ⚠️ 같은 종목의 여러 단계가 같은 날 겹치면(갭 하락) 이벤트는 단계별로 남기되 **체결·개방은 그 종목의
  첫 이벤트(carrier)에 합산**하고 나머지는 `note`로 그 사실을 밝힌다(행마다 나눠 실으면 같은 체결이
  두 번 표시된다). `reallocAmount`는 **날짜 단위 값**이라 그날 첫 매수 이벤트에만 싣는다.
- **화면**: 설정은 **`⑤-b 전략 옵션`** — ⑥ 종목을 ⑦로 밀지 말 것(엔진 경고가 '⑥ 종목에서…'를 가리킨다,
  가드 #243). 결과는 `StrategyKpis`(단일 뷰·비교 블록이 `SummaryCard`를 **공유**, 가드 #242), 비교 표에
  **최저 예수금·월분배 표준편차 2열 추가**(⚠️ 실행 불가 행의 `colSpan`을 함께 고칠 것 — 가드 #241이
  thead `<th>` 수와 대조한다). 월별 표는 **12열 유지**하고 `t.note`('바닥선'·'예수금 부족')는 날짜 셀
  **배지**로 붙인다(비고 열이 없어 그러지 않으면 화면 어디에도 안 보인다, 가드 #246).
...
- 검증: `npm run verify:backtest` (372건 — 참조 구현 미러 #1~#58·#69~#77·#81~#112·#113~#133·#141~#145·
  **#157~#226**·**#235~#240**·**#250~#256**(적대적 리뷰 회귀)·**#260~#274**(평가·메모)·
  **#300~#322**(시그널 당일 실행·매도 시그널·재원 3단계·분모 캐스케이드·적대적 리뷰 확정 결함 4건) + 소스 텍스트
...
- **게이트**(통과 못 하면 커밋 금지): 변경 영역의 `npm run verify:*`(calendar·tax·dividend·history·
  ...
  (.tsx 구문) · `undefcheck.mjs`(미정의 식별자). `npm run build`가 가능한 환경이면 추가로 돌린다.

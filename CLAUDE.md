...
4. **차이 열 = 확인분 차이**(D5): "전부 확인 시에만" → "확인 셀(실제·계획 둘 다 있는 셀) 1개 이상이면 그 셀들의
   ...
   총계가 같은 필드 → 소계 = Σ항목이 구조로 성립, `#107`). **롤업의 `unresolved > 0 → 산출불가 N`은 그대로**(R-3 —
   `loanSchedule` null 계약이 소계 경로에서 깨지지 않게, `#G11e-2`). `varianceOf` 셀 단위 null 계약도 그대로.
...
- 검증: `npm run verify:ledger` §8/§8b/#70/#89 재작성 + **§17 `#105~#119e`**(반영 항등식 5종·미분류·`confirmedOf`·
  사유 4종·`applyPlanAsActual` 8건·달력 `todayYm`·버킷 이동 — 기능마다 '동작 + 거래 0건 무영향' 쌍) + 가드 `#G10~#G10e`
  (부재 → 사용부 존재로 뒤집음)·`#G11b~#G11e-2`·`#G12/#G12b`·`#G27~#G27d`·`#G33b~#G33d`·`#G37c`·`#G37g`·**`#G39~#G39p`**,

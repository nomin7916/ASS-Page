...
푼다"는 운용을 재현하기 위한 `BtConfig` 6필드. **전부 정기 리밸런싱에만** 걸린다 — 초기 매수·이벤트
(구조 변경)·분배금 재투자는 종전 규약 그대로다(그쪽까지 번지면 PDF 재현 #23~#42가 깨진다).

| 필드 | 기본값 | 요약 |
|---|---|---|
| `band` | 0 | `\|리밸런싱 전 평가액 − 목표\| ≤ 목표 × band%` 면 그 회차 매매 생략 |
| `buyFunding` | `'both'` | `'tradeOnly'`면 평시 매수가 `cashTrade`만 쓴다(분배금 주머니 잠금) |
| `dip` | `{enabled:false}` | **가격 고점** 대비 −N% 첫 도달 시 `cashDiv × unlockPct`를 다음 리밸런싱 1회에 개방 |
...
- **⚠️ 급락은 평가액이 아니라 가격 고점 기준**(리밸런싱으로 수량이 계속 변해 평가액 고점은 왜곡된다).
  발동일은 사전 탐지로 확정하고 **개방액은 런타임 `cashDiv`에 달려 있어 `dip` 스텝에서** 계산한다
  (그래서 `KIND_ORDER`에서 `pay` 뒤·`rebal` 앞). 각 단계는 **고점 갱신 전까지 1회**, 새 고점이 서면
  전 단계 재무장. 발동 종목은 그 회차에 **밴드 면제**(급락 시 밴드로 매수를 막지 않는다). 개방은
  **그 회차 1회 한정**이라 매수가 없었거나 매도였어도 만료된다(가드 #232·#233).
- **⚠️ 매수는 매매 주머니를 먼저 쓰고 모자란 만큼만 개방분에서 꺼낸다** — 그래서 `dipEvents[].used`가
  0일 수 있다(정상). `buyFunding:'both'`에서는 개방이 무의미하므로 **로그만 남고 결과는 불변**이다.
...
- **KPI**: `minCash`(생존 판정 핵심) · `minCashDiv`(**첫 분배금 입금 이후**만 잰다 — 0에서 시작하므로
  ...
  달은 포함해야 '끊겼다'가 드러난다) · `bandSkipCount/Amount` · `dipEvents` · `shortfallMonths`.
...
- **적대적 리뷰 확정 결함 5건 (⚠️ 되돌리면 그대로 재발 — 검증 #250~#259)**:
  ① **`delta === 0` 회차에서 급락 개방이 만료되지 않았다**. `runPlan`은 `delta<0`/`delta>0` 두 필터로만
  불리는데, 목표에 이미 맞아떨어졌거나 종가가 이월된 회차는 어디에도 안 들어간다 → 개방액과 **밴드 면제가
  다음 회차까지 살아남았다**. 두 패스 뒤에 **남은 armed 종목을 훑어 만료시키는 루프**가 반드시 있어야 한다.
...
- **범위 밖(의도)**: 매매차익 과세 · 거래수수료 · 슬리피지 · 급락 단계 5개 초과 · undo/redo.
  `dip`은 리밸런싱 슬롯이 있는 종목에만 효과가 있다(개방만 하고 매수 기회가 없으면 만료되지 않고 남는다).
...
- 검증: `npm run verify:backtest` (340건 — 참조 구현 미러 #1~#58·#69~#77·#81~#112·#113~#133·#141~#145·
  **#157~#226**·**#235~#240**·**#250~#256**(적대적 리뷰 회귀)·**#260~#274**(평가·메모) + 소스 텍스트
  가드 #59~#68·#78~#80·#134~#140·#146~#156·**#227~#234**·**#241~#249**·**#257~#259**·**#275~#290c**(평가·메모 배선)).

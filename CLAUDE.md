...
**가계부 재설계(2026-09-05 설계, 단계 A 구현 완료 2026-09-06)** — 명세는 저장소 루트 `가계부_재설계_2026-09.md`.
~~거래 원장 레이어(단계 A)~~ **완료** → 예산 진행·고정지출 확인·월 마감(B) → 계좌·카드 청구주기·이체(C) →
대출 상환표·순자산(D) → 목표·가져오기·연도 정리(E) → 분석 확장(F). **한 세션에 한 단계**, 시작 프롬프트는 그 문서 §11,
결정 6건은 §9(단계 A는 **제안 기본값대로** 진행했다 — 할부 월 분배 · 스냅샷 거래 제외 · 상한 6,000 ·
거래 있으면 거래 탭 기본). 단계 A의 규약은 CLAUDE.md 가계부 절 '거래 레이어(transactions)'에 있다.
§12(단계 A 사후 검토)의 P1 4건·P2 7건·P3 4건은 **처리 완료**(`cb48154`).
**⚠️ 단계 B에 앞서 §13(화면 재설계 2차 — 첫 화면 '분석' · '수입 및 지출' 접힘 트리 · 계획 선반영, 2026-09-06 설계)을
먼저 구현한다** — 기본 탭 '분석', 탭 라벨 '수입 및 지출', 그룹·결제수단 소계가 항상 보이는 접힘 트리, 그리고
'반영값(실제 ?? 계획)' 규약을 분석·전월/전년 대비·달력·엑셀 ③까지 확장(G-2 불변식의 **의도적 개정** — §13.6).
사용자 결정 7건은 §13.7, 시작 프롬프트는 §13.9. 코드는 아직 무변경이다(설계 세션은 문서만 남겼다).

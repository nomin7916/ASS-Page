...
- **보류 행 표시 규약(3곳 통일)**: `dodAbsChange=null` + `%`도 `'-'`. `0.00%`로 단언하면 '변동 없음'과
  구분되지 않는다. 통합 추이표·**헤더 '오늘 수익' 카드**(⚠️ `?? 0`으로 삼키지 말 것)는 `'-'`,
  메모 달력은 줄 자체를 숨김. 헤더는 보류 시 원본 `ledgerFlow`로 "입금 ₩N 반영 대기"를 안내한다.
  `HistoryPanel`은 일간 지표를 **셀에서 내리고 툴팁으로만** 표시하므로(아래 '추이 표 셀 구성') 보류를
  `'전일대비: … 일간 지표를 보류했습니다.'` 문장으로 알린다 — 같은 null 계약, 표기 매체만 다르다.
- **적용 지점**: `useIntegratedData` `intMonthlyHistory`(통합 단일 소스) → 통합 추이표·**헤더 '오늘 수익' 카드**
  ...
  `(직전 기록일, 당일]` — 직전 '거래일'이 아니라 '기록일'이다). ⚠️ 이 memo는 셀 표시를 툴팁으로 내린
  뒤에도 **계속 유지**한다 — 현재 유일한 소비처가 누적 셀 툴팁이라 지우면 툴팁의 일간 지표가 사라진다
  (CSV는 이 memo가 아니라 `computeDailyMetricsSeries`를 **직접 호출**하므로 CSV 수치 자체는 불변). CSV는
...
- **지표별 역할 고정(섞지 말 것)**: 누적 원금대비 `(V−C)/C` = "내 돈이 불었나"(요약 카드·개별 계좌 추이
  표 3번째 열·통합 추이표 '원금대비' 열·차트 일반 모드) / 일간 보정 수익률 = "오늘 시장에서 벌었나"
  (통합 추이표 '전일대비'·'일간 손익' 열, 개별 계좌는 툴팁).
  ⚠️ **통합 추이표의 전일대비 열에 누적값을 넣지 말 것** — 애써 만든 입출금 보정이 사라진다. 개별 계좌
  패널은 열이 3개뿐이라 **한 열만 고를 수밖에 없어** 누적을 택한 것이고(사용자 결정, 아래 '추이 표 셀
  구성'), 열이 6개인 통합 추이표는 둘 다 유지한다 — 두 표의 3번째 열이 다른 지표인 것은 의도된 차이다.
- **`utils.ts` `resolveRecordPrincipal(effectiveValue, record, date, sortedHistAsc, principalProp)`가
  ...
  `exactHist` 분기(차트 '나의 수익률'), `HistoryPanel` `cumulativeByDate`(추이 표 '누적 수익률 · 수익금' 열).
...
- **추이 표 셀 구성 (2026-07 개편 — 되돌리지 말 것)**: 개별 계좌(`HistoryPanel`)는 열 3개뿐이라
  `일자 / 평가자산 / 누적 수익률 · 수익금` 3열이고, **3번째 열은 누적(원금대비) 전용**이다 —
  윗줄 `±N%`(`cum.rate`) + 아랫줄 `±₩누적수익금`(`cum.profit = 평가자산 − 투자원금`). 평가자산 셀은
  금액만 남기고 옛 `누적 ±N%` 배지는 **제거**(같은 값이 3열에 커졌으므로 중복).
  **일간 지표(전일대비 %·일간 손익 ₩)는 3번째 열 툴팁으로 강등**: 하루짜리 값이라 입금 기록일과
  평가액 반영일이 **하루만 어긋나도 부호가 뒤집힌다**(실측: 07/21 입금 ₩49,118,578 → 그날 −₩11,002,033,
  다음날 +₩7,385,947로 거울상. 이틀 합산은 실제로 손실). `shouldHoldDailyMetrics`가 잡지 못하는 크기
  (ΔV가 흐름의 50%를 넘김)라 **보류 로직으로는 해결 불가** — 그래서 지표 자체를 바꾼 것이다.
  ⚠️ 이 열을 다시 전일대비로 되돌리지 말 것(사용자가 "왜 필요한지 모르겠다"고 명시적으로 요청한 변경).
  ⚠️ 일간 계산 자체(`dailyMetricsByDate`)는 **살아 있어야** 한다 — 툴팁·CSV·통합 뷰가 같은 공용 함수를 쓴다.
- **⚠️ 입출금 금액 배지는 어느 화면에도 렌더하지 않는다(사용자 요청 — 되돌리지 말 것)**: 일간 지표는
  이미 흐름이 제거된 값이라 금액을 함께 띄우면 중복 노이즈다. 제거된 3곳 — 통합 추이표 전일대비 셀,
  `HistoryPanel`(옛 전일대비 셀), 헤더 '오늘 수익' 카드("입금 ₩N 제외"). 보류 안내는 **금액 없이**
  `'입금 반영 대기'`로만 표기한다(`'-'`가 왜 떴는지 알려주는 진단이라 문구 자체는 유지).
  `netFlow`·`ledgerFlow` **필드는 계속 계산·소비되므로 제거 금지** — 보류 판정과 툴팁이 쓴다.
  금액은 `HistoryPanel` **누적 셀의 툴팁(hover)** 에만 남긴다(상시 표시가 아니므로 허용).

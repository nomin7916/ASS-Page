...
- 검증: `npm run verify:flow` (308건 — 미러 #1~#26 + **팬/줌 #38~#44b** + **색 #45~#48b** +
...
- **재설계(2026-09) 단계 A부터 아래 3모듈이 더 붙는다**: `LedgerQuickEntry.tsx`(빠른 입력 바) ·
  `LedgerTxTab.tsx`(거래 탭) · `scripts/verify-ledger-tx.mjs`(거래 레이어 전담 검증).
  명세는 저장소 루트 `가계부_재설계_2026-09.md`(단계 B~F가 남아 있다).

#### 거래 레이어(transactions) — 단계 A (2026-09, ⚠️ 회귀 주의)

월 매트릭스(계획 레이어) **아래에** 날짜별 거래 원장을 깔았다. 조사한 예산 앱이 전부 거래를 저장
단위로 쓰고 매트릭스를 뷰로 두는데, 이 앱에는 그 레이어가 없어 변동비를 월말에 한 칸으로 합산해야
했다(재설계 문서 §2 W1). 저장은 `LedgerBook.transactions`이고 **영속화 신규 지점은 0곳**이다
(`LedgerBook` 안이라 App.tsx 7지점·sticky·스냅샷·브릿지가 자동 상속 — BtConfig 선례).

- **⚠️ 1급 계약: 거래를 한 건도 넣지 않은 장부는 종전과 1원도, 1픽셀도 다르지 않다.** 집계 함수는
  `ix`(거래 인덱스)를 **선택 인자**로 받고, 넘기지 않으면 수동 값만 보던 종전 코드와 동일하다.
  검증이 기능마다 '동작 케이스 + 거래 0건 무영향 케이스'를 쌍으로 고정한다.
- **⚠️ 실제 금액의 단일 소스는 `actualResolved(item, ym, ix)`다** — (항목, 월)에 거래가 한 건이라도
  있으면 **거래 합**, 없으면 수동 `actual[ym]`, 그것도 없으면 `null`(미입력). `actualOf`(수동 값만)는
  **이름·의미를 그대로 둔다**(레거시 계약·커밋 경로 불변). 화면·엑셀·달력은 전부 `actualResolved`로
  옮겼고, 거래가 있는 매트릭스 칸은 **읽기 전용**(클릭 → 거래 탭)이다 — 같은 숫자를 두 곳에서 고칠 수
  있으면 어느 쪽이 진짜인지 화면이 답할 수 없다.
  ⚠️ 가려진 수동 값은 셀에 `수동 N 무시됨` 배지로 **반드시 노출**한다(조용한 오적용 금지). 첫 거래가
  들어온 달에는 인라인으로 한 번 묻는다(`거래로 옮기기` = `origin:'migrate'` 1건 생성 / `수동 값 삭제`).
  **추가를 막지는 않는다**(입력 마찰 금지) — 모달·토스트도 쓰지 않는다(z-1090 + 별도 창에는 App이 없다).
- **⚠️ 집계 호출부가 `ix`를 빠뜨리면 거래가 조용히 무시돼 합계가 줄어든다** — 이 저장소가 가장 싫어하는
  실패 모드다. `#TG1~#TG1g`가 `LedgerPage`·`ledgerExcel`의 **사용부**를 하나하나 단언한다.
  ⚠️ `totalOf(items, k, ix)`는 월 셀과 연 루프 **두 곳**에 있어 파일 전역 정규식으로 재면 한쪽 되돌림이
  통과한다(실측 — `#TG1c`는 문맥까지 함께 본다).
- **`txIndexOf(book)` — O(거래 수) 1회 + `WeakMap` 참조 캐시**. 장부는 편집마다 새 객체이므로 참조가 곧
  정확한 무효화 신호다(제자리 변형 금지 — 캐시가 낡는다). 축 7종: `byItemYm`·`byYm`·`byDate`·
  `byAccountYm`(단계 C)·`uncategorizedYm`·`byPayerYm`·`firstTxYm`.
- **⚠️ 휴지통·이체는 인덱스에서 한 번 걸러진다**(소비자가 다시 신경 쓰지 않게):
  · 삭제는 **소프트**(`deletedAt`) — 되돌리기 없는 삭제는 이탈 원인이고 이 화면엔 확인 토스트가 없다.
    영구 삭제는 휴지통에서 한 번 더 눌러야 한다. **휴지통 제외는 방어선이 둘**이다(인덱스 진입부의 조기
    `continue` + `isLiveTx`) — 하나만 지우는 변이는 다른 하나가 잡으므로 `#T4c`가 `isLiveTx`를 직접 묻는다.
  · `kind:'transfer'`는 지출·수입 어디에도 들어가지 않는다 — 카드대금 결제·적금 이체가 지출로 이중
    계상되는 국내 자동연동 앱의 고질병을 **구조로** 막는 자리다(엑셀 시트 ④ 합계도 이체를 뺀다).
    같은 이유로 `filterTx`의 `uncategorizedOnly`는 이체를 제외한다(항목이 없는 것이 정상이라, 목록에
    섞이면 사용자가 항목을 붙이려 하고 그 순간 지출로 계상된다).
- **⚠️ 할부는 '이번 달 할부금이 이달 지출'**(사용자 결정 1의 기본값 — 똑똑가계부·같이가계부 규약).
  `installmentCharges(tx)`가 거래월부터 n개월로 나누고 **잔여는 마지막 회차가 흡수**해 Σ가 원금과 정확히
  같다. 발생주의(구입 시 전액)로 바꾸지 말 것 — 월 예산표 모델과 어긋난다.
  ⚠️ **날짜 축(`byDate`)만은 그날 결제한 전액**이다 — 회차의 날짜는 존재하지 않으므로 지어내지 않는다
  (달력에 사용자가 확인할 수 없는 값이 찍힌다). 전 기간을 통틀면 `Σ byDate === Σ byYm`이라 검산은 성립한다.
- **⚠️ 미분류(항목을 고르지 않은 거래)는 '기록해도 어디에도 없는 돈'이 되면 안 된다.** 빠른 입력이
  항목 없이 Enter를 허용하는 대가다 → `monthTotals`가 **`actualExpense`·`byGroup.variable`·`byPay`에
  더하고**, 매트릭스는 변동비 그룹 끝에 **미분류 가상 행**을 그리며, 소계·총계는 `extra` addend로 그
  몫을 함께 센다(`Σ그룹 === 총계`, `Σ결제수단 행 === 그룹 소계` 유지). 엑셀 시트 ①에도 같은 행이 있다.
- **⚠️ 분할은 `Σsplits === amount`가 아니면 정규화가 **강등**한다**(단일 항목으로). 화면은 아예 저장을
  거부하고 '남은 금액 채우기'를 준다 — 어긋난 채 저장되면 항목 합 ≠ 거래 합이 되어 어떤 검산도 통과하지
  못한다. 금액은 **항상 양수**이고 환급·취소는 `refund` 플래그다(부호와 플래그가 둘 다 뜻을 가지면
  집계마다 이중 부호 실수가 난다 — 음수 입력은 정규화가 `refund:true`로 옮긴다).
- **⚠️ 진행 중인 달을 '미입력'으로 세지 않는다**: `expectsActual(item, ym, {ix, todayYm})`가 `entry:'tx'`
  항목의 이번 달을 미입력에서 뺀다(그 배지는 한 달을 다 살기 전엔 끌 방법이 없어 상시 점등한다).
  ⚠️ `entry:'monthly'`(레거시)에는 적용하지 않는다 — 월 단위 입력은 이번 달을 미리 채우는 것이 정상 습관이다.
  `entry`는 **표시·안내 전용**이고, 거래가 있으면 값은 언제나 거래 합이 이긴다.
- **`mtdCompare(book, ym, todayDate, ix?)`** — 진행 중인 달의 **같은 기간 비교**(이달 1~오늘 vs 전달
  1~같은 일수). `compareMonths`(닫힌 달끼리, 미입력 집합 비교)는 **규약 그대로 둔다**.
  ⚠️ 전달이 짧으면 말일로 캡하는데, 없는 날짜는 조회해도 0이라 **금액만으로는 캡 제거가 관측되지 않는다**
  (죽은 단언이었다) → 결과에 `prevDays`를 실어 그 값을 단언한다. 화면 노출은 단계 B.
- **⚠️ 스냅샷(이전 기록)에는 거래를 넣지 않는다**(사용자 결정 2): `stripTxForSnapshot` + `txStripped`.
  512KB 예산에 거래(장부당 ≈700KB)를 넣으면 `pushLedgerSnapshot`이 이전 스냅샷을 전부 버려 **사실상
  1개만 남는다** — 여러 시점의 복구 지점이라는 존재 이유가 사라진다. 거래의 안전망은 **휴지통**이고
  스냅샷은 계획 매트릭스·수동 실적·설정의 안전망이다.
  ⚠️ 그 짝으로 **복원은 현재 거래를 지우지 않는다**(같은 id 장부에 현재 `transactions`를 다시 싣는다) —
  안 그러면 복구 수단이 데이터를 지우는 역설이 된다.
- **⚠️ 정규화 계약 2종**: ① `transactions`가 없던 레거시 장부는 **원본 참조 그대로**(멱등 — 매번 새 배열이면
  Drive 폴링마다 재저장 + 로컬 사본 갈아엎기로 2.5초 idle 승격 전 편집이 사라진다) ② 정규화는 **휴지통을
  비우지 않는다**(오늘 날짜에 따라 결과가 달라지면 멱등이 깨진다 — 비우기는 사용자 동작).
  상한 초과분은 **오래된 것부터** 버리고, `addTx`는 상한에서 **거부**한다(무엇을 버릴지는 사용자가 정한다).
- **지문**: `ledgerFingerprint`에 거래 **전 필드**(`createdAt` 포함 — 같은 날 같은 금액 두 건을 구분)와
  `items[].entry`를 투영하고, **`WeakMap` 참조 캐시**를 붙였다(저장 effect 첫 블록이라 거래 6,000건이면
  매 렌더 수 ms). 길이·개수 해시로 줄이지 말 것(그 절충이 '메모만 고치면 저장 안 됨' 버그를 두 번 냈다).
  `ledgerBooksHaveContent`도 거래를 본다 — 항목을 다 지우고 거래만 남긴 장부가 백업 복원에서 사라지면 안 된다.
- **쓰기 헬퍼는 전부 순수 함수 + id 기준**이고, 화면은 **`setLocal` 업데이터 밖에서 한 번만 계산**한다
  (`addTx`는 `generateId`·충돌 보고 같은 부수효과를 갖는다 — 업데이터 안에서 부르면 StrictMode 이중
  호출에서 두 번 돈다. FlowBoard·BacktestPage와 같은 규약).
- **달력 가계부 칩**: `kind:'tx'`(그 날 거래 합)가 **1순위**, 그다음 연단위·정리 기록이다("이 날 얼마
  썼나"가 달력의 첫 질문이고, 정리 기록의 금액은 그 달 전체라 날짜 칸의 뜻과 다르다). 라이브 파생이라
  `calendarMemos`에 복사하지 않고 `open` 게이트도 그대로다.
- **엑셀은 시트 4장**(④ 거래내역 — 날짜·종류·항목·구분·결제·누가·금액·할부·메모·출처·분할).
  분할은 **몫마다 한 행**(금액 열의 합이 곧 그 해 지출이라 피벗이 바로 돈다), 휴지통은 빼고, 할부는
  결제일 1행이다(월 배분은 시트 ①이 한다 — 두 시트가 같은 돈을 두 번 세지 않게).
- **UI 규약**: 탭 5개(`거래`·월 매트릭스·대출·분석·연간), 기본 탭은 **상위 `books`가 도착한 뒤 한 번만**
  판정한다(첫 렌더의 `book`은 시드된 빈 장부라 그때 재면 거래가 있는 사용자도 매트릭스로 열린다).
  빠른 입력 바는 **탭과 무관하게 하단 고정**(금액 → 항목 → Enter 3단계, 추가 후 금액·메모만 비움),
  `readOnly`면 **통째로 렌더하지 않는다**(끊긴 창에서 한참 입력한 뒤 아무 데도 저장되지 않는 것이 가장
  나쁜 결과다 — 엑셀 버튼과는 반대 규약). 신규 컴포넌트에 `notify(`·`window.confirm` **0건**.
- **범위 밖(단계 B~F)**: 예산 진행률·고정지출 ✔·자동 기입·다가오는 지출·월 마감·이월(B) / 계좌·카드
  청구주기·이체 UI·잔액 대조(C) / 상환표·순자산(D) / 목표·가져오기·규칙·연도 정리(E) / 구분 2단·주간
  보기·모바일 폭(F). `accountId`·`toAccountId`·`payer`·`originKey` 필드는 **모델에만** 있고 UI는 뒤 단계다.
- 검증: `npm run verify:ledger-tx` (**159건** — 파트① `#T0~#T45` 직접 import / 파트② `#TG1~#TG13c` 사용부
  가드). 기존 `verify:ledger`(**493건**)는 매트릭스·대출·엑셀 ①~③·팔레트 배선을 계속 지킨다.
  ⚠️ **변이 25종으로 실제 검출을 확인**했다(actualResolved가 인덱스 무시 · 휴지통 방어선 2개 동시 제거 ·
  이체 누출 · 할부 미분배 · 분할 불일치 미강등 · 미분류 제외 · 지문에서 거래 누락 · 레거시 멱등 파괴 ·
  스냅샷 미strip · 말일 캡 제거 · 매트릭스/엑셀이 `actualOf`로 되돌아감 · 소계 `ix` 누락 · 복원이 거래
  삭제 · 소프트 삭제가 영구 삭제로 · 엑셀 합계에 이체 포함 · 달력 칩 우선순위 되돌림 · `notify` 추가 ·
  충돌 미보고 · `filterTx` 휴지통 포함 · 진행중 게이트 제거 + **음성 대조 2종**(주석만 변경 · 방어선
  하나만 제거)). 그 과정에서 **죽은 단언 3건**(휴지통·말일 캡·소계 `ix`)을 찾아 고쳤다 —
  가드를 손볼 때 같은 변이가 여전히 잡히는지 다시 확인할 것.

#### 단계 A 사후 수정 — 축 정합 · 사라지는 돈 · 한 화면 한 규칙 (2026-09-06, ⚠️ 회귀 주의)

배포분(`9e40746`)을 다시 훑어 잡은 P1 4건·P2 7건·P3 4건. 재설계 문서 §12가 원본 재현 기록이다.

- **⚠️ 거래 `kind`와 항목 `group`은 **한 축**이다 — `resolveTxAxis` 하나가 강제한다(A1·B7)**:
  항목(또는 분할)이 있으면 `kind`는 **항목 group에서 파생**되고, `'transfer'`는 **항목이 비어 있을
  때만** 살아남는다. 과거엔 빠른 입력 바의 `지출/수입` 토글이 항목과 무관하게 눌려, 변동비 항목에
  수입 거래를 넣으면 `ix.byYm`은 수입 50,000·지출 0인데 `monthTotals`는 항목 group으로 판정해
  **지출 50,000·수입 0**을 냈다(같은 달이 두 값). 사용자가 클릭 두 번으로 만들 수 있는 상태였다.
  - **충돌은 '방금 만진 축'이 이긴다**(`updateTx`가 `patch`를 함께 넘기는 이유): `patch.kind ===
    'transfer'`면 항목·분할을 비우고, 그 밖에 **이미 이체인 거래에 항목을 붙이려 하면 이체가 이긴다**.
    ⚠️ 뒤쪽을 '항목이 이긴다'로 뒤집지 말 것 — 카드대금 결제·적금 이체가 지출로 되살아나
    `'transfer'`를 둔 이유(이중 계상 차단)가 통째로 무너진다. 일괄 재분류에 이체가 섞여도 안전하다.
  - **⚠️ `normalizeLedgerBooks`에서는 `sameTx`보다 **먼저** 정정한다.** `sameTx`는 정규화 결과를
    **원본**과 비교하므로, 먼저 정정해야 어긋난 레거시 행이 `txChanged`를 **딱 한 번** 세우고 그 뒤로는
    같은 참조가 유지된다(멱등 계약). 순서를 뒤집으면 `txChanged`가 영영 서지 않아 정정이 저장되지
    않고 매 로드마다 같은 계산만 반복한다.
  - 분할은 **첫 split 항목** 기준(타이브레이크 고정). 모르는 항목이면 **지어내지 않는다**.
  - 화면은 항목이 있으면 지출/수입 토글을 **비활성**하고 사유를 title로 밝힌다(빠른 입력·거래 탭 양쪽).
- **⚠️ 적용기간 밖이라도 거래가 있으면 센다 — `isItemCounted`(A2)**: 할부 회차는 거래월부터 n개월로
  나뉘어 `activeTo` **이후** 달에 떨어질 수 있다. 그 회차는 `ix.byItemYm`에는 있는데 `isItemActive`
  게이트에 막혀 `monthTotals`·`expectedTotal`·매트릭스·엑셀에서 전부 0이 됐다 — **돈이 조용히
  사라지는** 실패 모드(실측: `activeTo:'2026-10'` 항목의 11월 회차 10,000이 세 집계에서 0).
  - **⚠️ `isItemActive`를 넓히지 말 것** — 그 함수는 계획(`planOf`)·미입력 판정(`expectsActual`)·
    연단위 달력의 게이트라, 거래 유무로 활성이 바뀌면 "적용기간 밖인데 계획이 뜬다"가 생긴다.
    넓히는 곳은 **실적을 세는 소비자 5곳**뿐: `monthTotals` · `addExpected` · `expectedByPay`(자체
    게이트가 **따로** 있다 — 둘 다 아니면 둘 다) · `expectedOf` · 매트릭스 셀(+엑셀 매트릭스).
  - **⚠️ 판정을 `actualResolved(...).value !== null`로 넓히지 말 것** — 적용기간 밖에 남아 있는
    **수동** 값까지 딸려 들어와 거래 0건 장부의 동작이 달라진다. `source === 'tx'`라야 `ix` 없이
    구조적으로 `isItemActive`와 완전히 같다(하위호환의 축).
  - `expectsActual`은 **좁게 유지** — 그 달은 값이 있지만 '미입력'으로 세지 않는다. 매트릭스 셀은
    `-` 대신 **거래 합 + 앰버 '적용기간 밖' 배지**를 그린다.
- **⚠️ 매트릭스 셀 클릭은 '그 항목·**그 달**'을 연다(A3)**: `openTxFor(itemId, ym)`/
  `openUncategorized(ym)`가 `setYear`/`setMonth`를 함께 옮긴다. 거래 탭의 `scope:'month'`는 자기
  상태가 아니라 이 화면의 `year`/`month` prop으로 `makeYm`을 만들기 때문에, 안 옮기면 10월 칸(≡3건)을
  눌러도 9월 목록이 떠 화면이 자기 자신과 모순된다. ⚠️ `onClick={openUncategorized}` **맨 참조 금지**
  (합성 이벤트가 ym 자리로 들어간다).
- **⚠️ 스냅샷 목록이 '거래 제외'를 말한다(A4)**: `txStripped`를 저장·요약까지 실어 놓고 화면에
  렌더하지 않아, 복원한 사용자가 "거래가 안 돌아왔다"로 읽을 수 있었다. 목록 행에 배지 + 복원 확인에
  "거래는 그대로 유지됩니다(휴지통이 안전망)" + 각주. ⚠️ `ledgerSnapshotSummary`의 반환 shape은
  **손대지 말 것**(`verify:ledger #101`의 깊은 비교가 즉시 깨진다).
- **⚠️ 미분류는 분석 탭 카드에도 들어간다 — "한 화면 한 규칙"(B1)**: 매트릭스 소계·총계·KPI 배너·
  달력은 미분류를 더하는데 도넛 셋(메인·상세·결제수단 스트립)만 `expectedTotal`(항목 기준)이라
  **같은 달 합계가 카드마다 달랐다**. 메인 도넛은 **변동비 조각에만** 더하고(매트릭스 가상 행 위치·
  `monthTotals.byGroup.variable`과 같은 규약), 상세 도넛은 **`byKey`에 넣어 fold에 참여**시키며,
  `yearSeries.pay_*`가 `uncPayValue`를 더한다(스트립·막대·범례가 전부 여기서 파생).
  - **⚠️ 상세 도넛에서 미분류를 fold 뒤 별도 슬롯으로 붙이지 말 것** — 변동비 조각이 6개가 될 수 있어
    램프 인접 ΔE가 4 아래로 떨어지고 `verify:palette` §2가 단언한 n≤5 상한을 정면으로 위반한다.
  - **⚠️ `expectedGrandTotal`에 미분류를 더하지 말 것** — `#85b`(Σ결제수단 === 총액)가 깨지고
    G-2절이 그어 둔 '항목 축 vs 총계 축' 경계가 무너진다. 항등식의 **우변에서** 명시적으로 더한다.
  - **⚠️ TDZ — `uncTotalOf`/`uncPayValue`는 `ix` 직후·도넛 memo보다 **위**에 선언한다.** 그 memo
    팩토리는 렌더 중 동기 실행되므로 선언이 아래에 있으면 첫 렌더에서 `ReferenceError`가 나고,
    이 화면은 렌더가 try/catch로 감싸여 있지 않아 **가계부 탭이 통째로 오류 페이지**가 된다
    (`initTradeRest` 프로덕션 장애와 같은 부류 — build·undefcheck·scopecheck 어느 게이트도 못 잡는다).
- **⚠️ `entry` 전환은 `flipEntryToTx` 하나를 두 라이터가 공유한다(B2)**: `addTx`에만 있어서 거래 탭의
  항목 변경·일괄 재분류로 거래를 받은 항목이 `entry:'monthly'`에 머물렀고, 그 항목은 이번 달 거래가
  0건인 동안 계속 '미입력'으로 점등했다(끌 방법이 없다). 새 writer(단계 B autoPost·단계 E 가져오기)를
  추가할 때도 이 헬퍼를 부를 것. **`entry`는 계속 저장 필드다** — `ix.firstTxYm` 파생 전환은 사용자가
  **보류**로 확정했다(스키마·정규화·지문 3곳이 걸리고 사양 §5.3.2의 '되돌리기'에 새 필드가 필요하다).
- **⚠️ 엑셀도 화면과 같은 '미입력' 규칙을 쓴다(B3)**: `ctx.todayYm`을 만들어 `monthTotals` **두 호출**과
  매트릭스 `expectsActual`에 넘긴다. ⚠️ `expectsActual`에는 `ix`를 **함께** 넘겨야 한다 —
  `opts.ix` truthy 게이트 때문에 `todayYm`만 넘기면 조용한 no-op이다.
- **⚠️ 수동값 충돌은 **큐**다(B4)**: 단수 슬롯이라 거래를 연달아 넣으면 앞선 (항목, 월)의 질문이
  조용히 사라지고 셀 배지만 남았다. `itemId|ym`로 중복만 거르고, **'나중에'는 한 건만 넘긴다**
  (큐를 비우면 이 결함이 그대로 재발). `migrateManualToTx`는 거래 상한에서 **같은 book 참조**를
  돌려주므로 성공과 구분되지 않아 프롬프트만 조용히 닫혔다 → **상한을 먼저 보고** 실패는 닫지 않고
  플래시로 사유를 낸다. 큐에 실린 뒤 이미 해소된 건은 조용히 넘긴다(staleness 가드).
- **⚠️ 달력 패드가 그 날 거래 목록을 보여 준다(B5)**: 이벤트에 `txs`(최대 `LEDGER_CAL_TX_CAP` 20건)를
  **1회 순회 Map**으로 싣는다(날짜마다 `filterTx`를 부르면 O(날짜 × 거래)라 6,000건에서 폭발).
  - ⚠️ 술어는 `byDate`와 **문자 그대로 같아야** 한다(`isLiveTx` + 이체 제외 + 그 해) — 다르면 칩의
    'N건'과 줄 수가 어긋나고, 이체가 새면 카드대금 결제가 "이 날 이만큼 썼다"로 읽힌다.
  - ⚠️ `amount`는 **저장값 그대로 양수**(부호는 `refund`) — 미리 곱하면 그 계약이 이벤트 경계에서
    깨져 이중 부호 실수가 재발한다. `kind`를 빼지 말 것(`txCount`는 수입도 세므로 급여가 지출 행이 된다).
  - ⚠️ 절단은 **밝힌다**(`외 N건`) — 목록만 자르고 `txCount`는 전 건을 유지하므로, 안 밝히면 합계와
    목록이 어긋나 보이는데 사용자가 원인을 알 수 없다. 이름은 `txDisplayName` 공유(거래 탭과 같은 값).
- **⚠️ 빠른 입력에서 없는 항목을 그 자리에서 만든다(B6)**: 일치가 없으면 친 이름이 통째로 버려지고
  거래만 미분류로 남았다. 제안 목록 마지막에 `새 항목 "…" 만들기(변동비)`.
  - **⚠️ `submit(full, forcedItemId)` — `setItemId(id)`는 비동기다.** 인자 없이 `submit()`을 부르면
    stale한 `itemId === ''`를 읽어 **미분류로 저장**되는데 화면에는 이름이 보여 고쳐진 것처럼
    착각하게 된다(가장 나쁜 실패 모드).
  - **⚠️ `handleCreateItem`은 `curBook()`(=`localRef.current`)을 읽고 `makeLedgerItem`을 업데이터
    **밖**에서 부르며 `readOnly`를 **직접** 막는다.** `setLocal`이 readOnly에서 no-op이라 가드가 없으면
    **추가된 적 없는 항목의 id**를 돌려주고, 그 거래는 존재하지 않는 항목을 가리킨 채 미분류로
    떨어진다(지금보다 나쁘다). `setLocal`이 `localRef.current`를 **동기로** 갱신하므로 같은 tick의
    `handleAddTx`가 새 항목을 본다.
- **P3(안전한 4건)**: `payer` 입력 `maxLength`(정규화가 20자에서 조용히 자르던 것) · `handleDeleteTx`가
  `today` 없을 때 플래시(별도 창 부팅 직후 "삭제가 고장난 것"처럼 보였다) · 환급 초과 달의 음수 셀
  툴팁 · **거래 추가 경로에서 `touchMonth` 생략**(그 기록은 '오늘 날짜'에 찍혀 **과거 달 거래를 넣어도
  오늘 칸에** '그 달을 정리했다' 줄이 생겼다 — 거래 이벤트가 이미 그 뜻이다). ⚠️ 매트릭스 수동 입력
  경로(`NumCell onCommit`)의 `touchMonth`는 **그대로 둔다**(날짜 없는 월 단위 입력이라 유일한 앵커다).
- **범위 밖(단계 B~F로 이월)**: 빠른 입력 날짜 키보드 단축키(`←/→`·`T`) · 거래 탭 가상화 ·
  480px 2줄 접힘. B2의 `entry` 파생 전환도 보류(위 항목).
- 검증: `npm run verify:ledger-tx` (**240건** — 신규 `#T46~#T51d`(§11 축 정합 · §12 isItemCounted ·
  §13 이체 · §14 entry · §15 달력 목록 · §16 새 항목) + `#TG14~#TG24e` 배선 가드) ·
  `npm run verify:ledger` (**499건** — 신규 `#83d~#83h` 미분류 항등식, `#G13d-2`·`#G13d-4`·`#G32c` 갱신).
  ⚠️ **변이 44종 + 음성 대조 1종으로 검출을 실증**했다(파생 3지점 각각 · 정정↔`sameTx` 순서 뒤집기 ·
  이체 분기/항목 비우기 제거 · 분할 타이브레이크 뒤집기 · 게이트 되돌림 5지점 · `isItemCounted`를 값
  기준으로 넓힘 · 배지 무력화 · `flipEntryToTx` 제거 · 미분류 3지점 각각 · 엑셀 `todayYm` **두 호출을
  따로** · `expOpts` 제거 · 큐→덮어쓰기 · '나중에'가 큐 비우기 · 상한 체크 제거 · `forcedItemId`
  미전달 · prop 배선 제거 · 이벤트 `txs`/이체 술어/캡/패드 목록/절단 고지 · `txDisplayName` 손복제 ·
  달 이동 3지점 · 스냅샷 배지·문구 · `payer` 상한 2곳 · 삭제 무피드백 · 환급 설명).
  ⚠️ 그 과정에서 **내 테스트의 죽은 단언 1건**을 찾아 고쳤다 — `txs` 필드를 지우는 변이에서
  `d5.txs.length`가 **TypeError로 스크립트를 죽여** 하네스가 '미검출'로 오분류했다. §15는 전부
  옵셔널 체이닝 + 기본값으로 바꿔 **값 단언이 실패로 떨어지게** 했다(`#T2`의 같은 규약).
  ⚠️ 하네스는 **두 스크립트가 모두 정상 요약을 냈을 때만** 검출로 센다.
...
신규 지점 0곳. **이 절은 위 '거래 레이어'·아래 '계획 자동 반영'·'예상(expected) 집계' 절의 몇 항목을 의도적으로
개정한다**(§13.6) — 되돌리면 첫 화면(분석)의 ②·연간·달력이 다시 대부분 `-`가 된다(요청의 원인).

1. **G-2 절 개정 — 반영값(실제 ?? 계획 + 미분류)은 `reflectedMonth`·`reflectedCompare` 전용 함수로만 소비한다.**
...
2. **옛 −87.2% 사고의 방어는 산식이 아니라 구조로 남는다** — 두 달이 모두 반영값이라 완비되고 '완료도 불일치'라는 상태
   ...
   읽힌다.** ①②는 이번 달 `entry:'tx'` 거래 0건 항목(`confirmedOf().inProgress`)까지 센다 — `expectsActual`이 그 항목을
   미확인에서 빼는데 값은 계획으로 채워지므로, 빼먹으면 헤더 `✓`와 반영값이 정면 모순이다(§13.11 R-6, `#109c`·`#G39n`).
3. **롤업 셀의 `계획 N` 배지 → 월 헤더 `확인 N/M`으로 대체.** 배지를 되살리지 말 것(사용자가 소음으로 지목 — `#G11c`·
   ...
   소스는 `confirmedOf(book, ym, todayYm, totals?)`(`monthTotals.missingExpense` + `inProgress`), `yearSeries` 행이
...
4. **차이 열 = 확인분 차이**(D5): "전부 확인 시에만" → "확인 셀(실제·계획 둘 다 있는 셀) 1개 이상이면 그 셀들의
   ...
   미분류 행은 계획이 없어 `-`(확인분 차이에 들어가지 않는다, `#108b`).
...
4c. **계획 열 숨기기 · 열 구분선 (2026-09 사용자 요청)**: `계획` 헤더 위 4px 띠 = 월 열과 **같은 UX**로 열 숨김,
   ...
   ⚠️ 계획 셀은 **4렌더러**(헤더·항목·미분류·롤업)에 있고 **전부** 같은 게이트를 지나야 한다 — 한 곳만 빠지면
...
5. **기본 탭 = 상수 `'chart'`**(§9 결정 4 폐기, `tabSeededRef` 삭제 — `#G39b`·`#TG12c`). 탭 순서 분석·수입 및 지출·거래·
   대출·연간, 키 `'matrix'`·엑셀 시트명 '월 매트릭스'는 유지(`#TG12`·`#80b`).
6. **트리 매트릭스** — L0 그룹 행(항상, 항목 0건이어도) → L1 결제수단 행(2종 이상이면 **항상 노출**, D3) → L2 항목 행
   ...
   편다(`openBucket`, `#G39m` — 만든/옮긴 행이 안 보이면 실패로 읽힌다). 미분류 가상 행은 수단 하위에
   `renderUncategorizedRow(p)`(`#G39c-3`). 롤업 숫자는 `fmtNum`(콤마·₩ 없음 — 검산 화면이라 축약 금지), 미래 달은 흐린
...
8. `mtdCompare`(거래 기반 같은 기간)는 그대로(단계 B 예산 탭이 쓴다). §5.3.3 "`compareMonths`는 닫힌 달끼리만" 문장은 폐기.
9. **'이 달 계획대로 확인'(`applyPlanAsActual`)은 계획값을 반올림 없이 저장한다**(R-1 — `#G39i`가 본문의 `Math.round`
   ...
   `monthTotals.missingIds`(지출 전용 — 수입은 구조적으로 제외, R-4) ∩ `planOf ≠ null` ∩ `entry ≠ 'tx'`(거래 입력 항목은
   단계 B의 거래 기반 ✔로). 프롬프트는 **실제로 쓰는 건수**를 말한다(`미확인 N건 중 W건 … (거래 입력 K건 · 산출 불가 J건
   제외)`). 멱등(같은 참조 → dirty 없음) · `ym > todayYm` no-op · 성공 시 `touchMonth` 1회 · 인라인 2단계 · `readOnly`/
...
11. **비교 가능 판정은 산출 불가 '집합'을 본다** — 같으면 그 항목을 양쪽에서 빼고 비교(`unresolvedExcluded` 라벨), 다르면
    ...
    `no-data`는 **양쪽** `activeCount`(+미분류)를 본다 — cur가 빈 달이면 −100%가 확정 표기된다(R-8, `#110b`·`#G27b`).
    사유 순서 `no-data → future → unresolved → zero-base`(`#G27c`). `zero-base`는 comparable=true·rate=null(엑셀 #89 계약).
- **분석 ① 스택 2단**: `confirmed`(실제 입력 + 미분류, `LEDGER_BALANCE_COLOR.expense`) + `planned`(계획 반영 — 같은 hue의
...
- **영속화 신규 지점 0곳** — 펼침 상태·`confirmYm`은 세션 로컬, 유일한 쓰기는 기존 `actual[ym]`(지문 포함). `LedgerCalendarEvent`에
  `reflectedExpense`·`unconfirmed`·`inProgress`·`momReason`·`momPlanned`가 늘었지만 파생 타입이다(저장 아님).
- **범위 밖(의도)**: 단계 B(예산 진행률·다가오는 지출·월 마감·이월·거래 기반 ✔ — 그때 이 버튼을 흡수), 펼침 상태 영속화,
  롤업 셀 클릭 → 거래 탭, 엑셀 ① 행 배치, 분석 카드 재배치, 헤더 KPI 6장 기준(계획 기준 유지).
...
- 검증: `npm run verify:ledger` §8/§8b/#70/#89 재작성 + **§17 `#105~#119e`**(반영 항등식 5종·미분류·`confirmedOf`·
  사유 4종·`applyPlanAsActual` 8건·달력 `todayYm`·버킷 이동 — 기능마다 '동작 + 거래 0건 무영향' 쌍)
  ...
  `npm run verify:ledger-tx` `#T20~#T20d`·`#TG6`·`#TG12c`, `npm run verify:palette` §7.
  ...
  **변이 33종 + 음성 대조 2종(주석만 변경 → 미검출)으로 검출을 실증**했다: 기본 탭 복귀 · 라벨 복귀 · 그룹 행 손계산 ·
  결제수단 행 삭제 / 그룹 펼침 안으로 · 미분류 행이 수단 무시 · 미래 달 이탤릭 제거 · ① 스택 한 막대 / 미래 달 막대 그림 ·
  ...
  게이트 되돌림 · 계획 N 배지 부활 · inProgress 제거 · future 게이트 제거 · 헤더 진행 K 제거 · 확인분 차이가 미확인 셀까지.
...
#### 엑셀(.xlsx) 내보내기 — 시트 4장 (⚠️ 회귀 주의)

헤더의 **⭳ 엑셀** 버튼(⧉ 새 창 왼쪽)으로 **보고 있는 연도**를 시트 4장짜리 통합문서로 내려받는다.
`src/ledgerExcel.ts`(빌더) + `src/xlsxWriter.ts`의 **`buildXlsxMulti`**(다중 시트 확장).

| 시트 | 열 | 내용 |
|---|---|---|
| 월 매트릭스 | 29 | 결제·항목·계획 + [실제·차이] × 12개월 + 연간[실제·차이] · 그룹 섹션/소계/총계 · **미분류 행** |
| 대출 | 12 | 사진의 표 + 잔여 회차 + **값 출처** · 연 납입액(향후 12개월) · DSR |
| 연간요약 | 7 | KPI 9종 + 월별(계획·실제·차이·전월대비·미입력) + 구분별 + 결제수단별 |
| 거래내역 | 11 | 날짜·종류·항목·구분·결제·누가·금액·할부·메모·출처·분할 (거래 레이어, 단계 A) |
...
**가계부 재설계(2026-09-05 설계, 단계 A·§13 구현 완료 2026-09-06)** — 명세는 저장소 루트 `가계부_재설계_2026-09.md`.
~~거래 원장 레이어(단계 A)~~ **완료** → ~~§13 화면 재설계 2차~~ **완료**(`6e4166e`) → **다음 = 예산 진행·고정지출 확인·
월 마감(B)** → 계좌·카드 청구주기·이체(C) → 대출 상환표·순자산(D) → 목표·가져오기·연도 정리(E) → 분석 확장(F).
**한 세션에 한 단계**, 시작 프롬프트는 그 문서 §12.5(단계 B), 결정 6건은 §9(단계 A는 **제안 기본값대로** 진행했다 —
할부 월 분배 · 스냅샷 거래 제외 · 상한 6,000. 4번 '거래 있으면 거래 탭'은 §13이 **폐기** — 기본 탭은 상수 '분석').
§13은 결정 D1~D7 전부 제안대로 구현했다(규약은 CLAUDE.md 가계부 절 '화면 재설계 2차(§13)'). ⚠️ 단계 B의
'고정지출 ✔ 확인/자동 기입'이 들어오면 §13.2.4의 '이 달 계획대로 확인' 버튼을 그 워크플로에 **흡수**한다(병존 금지).

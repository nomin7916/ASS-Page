...
1. **`refreshPrices` 함수** (~80줄) — `useStockData` 훅으로 이동 가능
2. **`applyStateData` 함수** (~60줄) — `useDriveSync` 훅으로 이동 가능
3. **인라인 JSX 컴포넌트** — `StatusDot`, `CompStockDot` 등 소형 컴포넌트들
4. **탭 바 / 계좌 선택 UI 블록** — 별도 `AccountTabBar` 컴포넌트로 분리 가능
5. **PIN 관련 상태 + 핸들러** — `usePinManager` 훅으로 묶기 가능

...
**매도 실현손익 — '현재 평가 / 실현손익' 열 (⚠️ 회귀 주의 — 기준단가를 과세열과 통일하지 말 것)**

매수 행이 그 열에 **미실현** 평가손익(현재가 × 수량 − 매입금액)을 보여주는 것과 짝을 이루는
**실현**손익. 매도 행에 `매도금액 − 매입원가 = (매도단가 − 기준단가) × 매도수량`을 3줄로 렌더한다
(금액 / 주당손익·수익률 / 그 행에 쓰인 기준단가). 사용자 요청 2026-08 "매도시 얼마의 수익을 냈는지
매수시처럼 표현".

- **⚠️ 기준단가는 '매도 시점 러닝 평균 매입단가'다 — 과세열의 `avgBuy.value`(현재 시점 포트폴리오
  구매단가)로 통일하지 말 것.** 실현손익은 **확정된 과거 사실**이라 나중 매수로 소급 변경되면 안 된다.
  되돌리면 재발하는 오류 2종(둘 다 부호가 뒤집힌다):
  ① **데이터가 정상이어도** 100주@8,000 매수 → 50주@9,000 매도 → 100주@12,000 매수 순서에서
  참값 **+50,000**이 현재 평균(10,666.67) 기준으로는 **−83,333**이 된다.
  ② 매도를 포트폴리오 표에 반영할 때 **보유수량만 줄이고 투자금액을 그대로 두면** 구매단가가 폭등해
  (8,000 → 21,621) 이익 매도가 **−8,417,822 손실**로 표시된다(이 표가 `avgBuy.verify` 경고를 두는 이유).
  → 그래서 **과세 3열은 종전대로 `avgBuy.value`**(그쪽의 '현재 시점' 규약은 사용자 선택), **실현손익만
  러닝 평균**으로 갈라 놓고, 두 기준의 주당 금액이 다를 수 있다는 사실을 **각주 + 셀의 '기준 N' 줄**로
  상시 노출한다(5% 이상 벌어지면 셀에 ⚠).
...
- **러닝 평균은 `buildSortedEventsWithAvg`가 평균 과표와 **같은 순회**에서 낸다**(`runningBuyAvg`).
...
- **⚠️ 같은 날짜 이벤트는 `compareTaxEvents`(매수 우선)로 결정적 정렬** — `localeCompare`만 쓰면
  ...
  ⚠️ 러닝 평균을 만드는 **모든 순회가 이 비교자를 공유**해야 한다 — `buildSortedEventsWithAvg`(계산기 행)와
  `computeRunningAvgSnapshots`(월별 그리드)가 다른 순서를 쓰면 **같은 달의 평균 과표가 두 값**이 된다.
...
- **범위 밖(의도)**: 세율 적용 세액 · FIFO 원가법 · 매도 요약의 미실현분 합산(총손익 = 실현 + 미실현
  카드는 만들지 않는다 — 이중 계상 혼선).
- 검증: `npm run verify:tax` (71건 — §9 `resolveAvgBuyPrice` · §10 `computeSellTaxRow` · **§11
  `computeSellRealized`** 참조 구현 미러 + **§12 렌더 배선 소스 텍스트 가드 #G1~#G11**).
  `src/krEtfTaxHelpers.ts` 본문과 `scripts/verify-kretf-tax.mjs`의 미러를 **항상 1:1 동기화**할 것
  (**#G11이 `computeSellRealized` 본문을 문자 단위로 대조**해 한쪽만 고친 드리프트를 잡는다 —
  ⚠️ 파라미터 구조분해 `{ change, ... }`를 본문으로 오인하면 어떤 드리프트도 못 잡는다).
  ⚠️ §12 가드는 **선언이 아니라 사용부**를 단언한다(셀 통째 삭제·값 바꿔치기·색 바인딩 제거를 잡아야
  한다). 나아가 **'존재'만으로는 부족한 계약 3종은 별도 방식으로 단언한다** — 우선순위는
  ...
  **추가**가 전부 71/71로 통과한다. 변이 **20종**(1차 11 + 2차 9)으로 검출을 실증했으므로, 가드를 손볼 때
  같은 변이가 여전히 잡히는지 다시 확인할 것.
...
- **호가별 등락률 열 = '전일 종가' 기준 (⚠️ 회귀 주의 — 현재가 대비로 바꾸지 말 것)**:
  ...
  - 값 전달은 `RebalancingPanel`의 `changeRate: item.changeRate ?? null` 하나 — 표의 등락률 열과
    **같은 소스**라 두 화면이 갈리지 않는다. 해외계좌는 USD 가격의 등락률이라 환산이 없다.
- **영속화 지점 0곳** — 사다리는 계산기일 뿐 아무것도 저장하지 않는다(호가·배수·행 편집 전부 모달
  로컬 state). `portfolioStructureKey`·`applyStateData`·`applyBackupData`·저장 effect deps 전 지점 무관.
- **범위 밖(의도)**: 실제 주문 전송, 호가단위 규정(가격대별 틱) 자동 적용, 수수료·세금, 부분 체결 추적.
- 검증: `npm run verify:ladder` (참조 구현 미러 #1~#45 + **금액 앵커 #58~#77** + **전일 대비 등락률
  #80~#91·#99b·#99c** + 배선 가드 #46~#57·#72~#73·#78~#79·**#92~#100**).

...
- **장부액 관측(`bookDelta`) — 흡수 판정의 1순위 근거 (⚠️ 제거하지 말 것)**: `bookCostOf` =
  ...
  **공급자 = 활성 계좌 단일 Map(`App.tsx` `activeBookByDate`)을 3소비자가 공유** — `accountTwrByDate`
  ...
  - **⚠️ 통합에서는 ACTIVE 자가치유(2행)가 꺼진다 — 의도된 맞바꿈**: `computeDailyMetricsSeries`의
    ...
    ⚠️ 2026-09부터 그 15행이 통째로 `'-'`가 되지는 않는다 — 장부가 흐름 방향으로 전혀 안 움직인 행은
    '미반영 확정'으로 보고 그날 시장 손익을 그대로 낸다(위 `UNREFLECTED_RATIO` 항목). 이월은 그대로다.
...
- **⚠️ ACTIVE 폐기는 `bookDelta == null`일 때만**: 관측이 있으면 '미반영'이 확정 사실이므로 폐기하면
  ...
  무한 이월을 막는다.
- **⚠️ 미반영이 '관측으로 확정'된 행은 숨기지 않는다 (`UNREFLECTED_RATIO` 5%, 2026-09 — 되돌리지 말 것)**:
  보류(`'-'`)의 존재 이유는 **흐름이 V에 들어왔는지 모르기 때문**이다. 그런데 `bookDelta`가 "장부가 흐름
  방향으로 한 푼도 안 움직였다"(≤ `|흐름| × 5%`)고 말하면 그건 모름이 아니라 **확정**이고, 그날 ΔV는
  순수 시장 변동이므로 그것이 곧 정답이다 → `dodAbsChange = ΔV`(흐름을 빼지 않는다) · `dodChange =
  ΔV ÷ 전일V` · `ledgerFlow = 0` · `held = false`.
  - **발단(사용자 보고 2026-09)**: 위 (B)를 고치면서 관측 모드에서 ACTIVE 폐기를 껐는데, 원장 기록일과
    예수금 수정일이 어긋나는 것은 **구조적 정상**이라 그 구간이 `CARRY_MAX_ROWS`(15행)까지 통째로
    `'-'`로 잠겼다(실측: 개별 계좌 추이표 최근 3행 연속 `'-'`). **알고 있는 시장 손익을 몇 주간 숨기는**
    상태였다.
  - **⚠️ 이월(`carryIn/Out/Ledger`·`carryRows`·`activeRows`)은 한 줄도 건드리지 않는다** — 흐름은
    종전대로 다음 행으로 넘어가고 **실제로 반영되는 날 정산**된다(#29c의 '반영일 ₩0' 불변식 유지).
    바뀌는 것은 '그 사이 행을 숨길 것인가' 하나뿐이다.
  - **⚠️ ΔV로 판정하지 말 것** — 그건 다시 추측이다. `bookDelta != null`일 때만 확정으로 본다
    (`bookDelta` 미제공 = 해외·추정 구성이면 **종전 동작 그대로**).
  - **⚠️ ΔV = 0(비거래일 carry-forward)은 제외** — 시장 정보가 아예 없는 행이라 `0.00%`로 단언하면
    '변동 없음'과 구분되지 않는다(아래 보류 행 표시 규약).
  - **⚠️ 부분 흡수(5%~50%)는 여전히 '모름'이라 보류 유지** — 확정 구간과 회색 구간을 합치지 말 것.
...
- **보류 행 표시 규약(4곳 통일)**: `dodAbsChange=null` + `%`도 `'-'`. `0.00%`로 단언하면 '변동 없음'과
  ...
  `HistoryPanel`의 `'-'` 아래에는 **금액 없이** `반영 대기` 한 줄을 덧붙인다(터치 기기에는 hover 툴팁이
  없어 `'-'`만 보이면 고장으로 읽힌다 — ⚠️ 금액 표기는 금지, '입출금 금액 배지 상시 렌더 금지' 규약).
...
- 검증: `npm run verify:twr` (명세 테스트 #1~#5 + 엣지 #6~#16 + 회귀 #17~#21c + 장부액 관측 #29~#29d
  + **미반영 확정 행 #29c·#29e~#29h**(확정이면 시장 손익 산출 · 부분 흡수는 보류 · 비거래일은 보류 ·
  장부 미제공은 종전 동작) + **드리프트 가드 #29i**(`src/utils.ts`를 직접 import 해 미러와 대조 —
  ⚠️ 미러만 고치거나 src만 고친 변경이 **둘 다 통과**하던 구멍을 막는다)

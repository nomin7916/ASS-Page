...
- **지표별 역할 고정(섞지 말 것)**: 누적 원금대비 `(V−C)/C` = "내 돈이 불었나"(요약 카드·추이 표
  누적 열·차트 일반 모드) / 일간 보정 수익률 = "오늘 시장에서 벌었나"(추이 표 전일대비 열).
  전일대비 열에 누적값을 넣지 말 것 — 애써 만든 입출금 보정이 사라진다.
- **`utils.ts` `resolveRecordPrincipal(effectiveValue, record, date, sortedHistAsc, principalProp)`가
  ...
  `exactHist` 분기(차트 '나의 수익률'), `HistoryPanel` `cumulativeByDate`(추이 표 '누적' 열).
  ...
  자동 일치**한다.
- **해외(overseas)는 USD 기준 별도 분기**: 원금 필드도 평가도 USD라 환산하지 않는다(`App.tsx` 해외
  분기와 동일). 수익금 ₩은 통화가 섞이므로 표시하지 않고 %만 낸다. `cumulativeByDate`는
  `overseasEvalByDate.get(date).usd` vs `cleanNum(principal)`.
- **⚠️ 조회시작 0% 모드(`isZeroBaseMode`)의 라인은 수익률이 아니라 '평가액 증감률'**: `App.tsx:1146`이
  `principalReturnRate`를 `평가액 ÷ 조회시작 평가액 − 1`로 덮어써, **구간 중 입금액이 그대로 분자에
  들어간다**(실측: 12.5M→108.2M이 **+761.57%**로 표시됐지만 그중 99.2M이 입금이라 실손익은 **−3.55M**).
  → `PortfolioChart`의 컴포넌트 스코프 `myReturnLabel = isZeroBaseMode ? '평가액 증감' : '나의 수익률'`
  하나로 **범례(Area name)·정보패널·계산식 검증 패널·버튼 title**을 통일하고, 흐름이 있으면
  `⚠ 순입금 N 포함 · 실손익 M`(= `endEval − startEval − 순흐름`, `externalFlowInRange`로 (시작일,종료일]
  반개구간)을 함께 노출한다. **이 라벨을 '나의 수익률'로 되돌리지 말 것.**
  ⚠️ **`chartUtils.tsx`의 `[구간: x%]` 태그 매칭이 이 문자열을 본다** — `'평가액 증감'`은
  `selectionResult.rate`(평가액 비율)로, `'나의 수익률'`은 `principalReturnRateAtEnd`(원금대비)로 갈라야
  라인·정보패널과 일치한다. 라벨 변경 시 양쪽을 같이 고칠 것.
- **추이 표 셀 구성**: 개별 계좌(`HistoryPanel`)는 열 3개뿐이라 평가자산 셀 = 금액 + `누적 ±N%`,
  전일대비 셀 = `%` + `±₩일간손익`으로 **병기**한다. **일간 손익 ₩을 툴팁에서 셀로 승격**한 이유는
  "실제로 수익 내고 있나"에 답하는 건 %가 아니라 금액이기 때문. `dodAbsChange == null`(보류)은
  여전히 `'-'`(위 null 계약 준수).
- **⚠️ 입출금 금액 배지는 어느 화면에도 렌더하지 않는다(사용자 요청 — 되돌리지 말 것)**: 일간 지표는
  이미 흐름이 제거된 값이라 금액을 함께 띄우면 중복 노이즈다. 제거된 3곳 — 통합 추이표 전일대비 셀,
  `HistoryPanel` 전일대비 셀, 헤더 '오늘 수익' 카드("입금 ₩N 제외"). 보류 안내는 **금액 없이**
  `'입금 반영 대기'`로만 표기한다(`'-'`가 왜 떴는지 알려주는 진단이라 문구 자체는 유지).
  `netFlow`·`ledgerFlow` **필드는 계속 계산·소비되므로 제거 금지** — 보류 판정과 툴팁이 쓴다.
  금액은 `HistoryPanel` 전일대비 셀의 **툴팁(hover)** 에만 남긴다(상시 표시가 아니므로 허용).
  **예외 1곳** — `PortfolioChart` 조회시작 0% 모드의 `⚠ 순입금 N 포함 · 실손익 M`은 유지한다.
  그 모드의 %는 입금액이 분자에 들어가 부호까지 틀리므로(+761% vs 실손익 −3.5M), 이 문구를 빼면
  화면이 다시 거짓말을 한다 — 배지가 아니라 **오표시 정정 고지**다.
- **파생값이라 영속화 무관**: `resolveRecordPrincipal`·누적 %·실손익은 전부 매 렌더 파생값이다.
  `portfolioStructureKey`·`applyStateData`·`applyBackupData`·저장 effect deps **전 지점 무수정**.
- **범위 밖(의도)**: 누적 TWR 곡선·XIRR은 **미적용**. 곱셈 체인은 하루짜리 오류를 영구 고정하므로,
  입금 원장 날짜와 평가 스냅샷이 어긋나는 데이터 갭(입금 당일 예수금·보유수량 미반영 → `calcPortfolioEvalDetail`이
  예수금 시계열 없이 현재 `depositAmount`를 과거 전 날짜에 사용)을 먼저 해소한 뒤 도입한다.

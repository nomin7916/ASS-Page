...
### 현재 상태 (Phase 9 완료 후)
- `App.tsx`: **약 2,280줄**
- `hooks/`: 7개 (`usePinManager` 추가)
- `components/`: 20개 (`AccountTabBar` 추가)
...
1. **`refreshPrices` 함수** (~150줄) — `useStockData` 훅으로 이동 가능 (setMarketIndices 등 복합 의존성 있어 복잡)
2. **`applyStateData` 함수** (~75줄) — 이미 ref 패턴으로 useDriveSync에서 호출 중; 내부 이동 시 setState 함수 30개 이상 전달 필요
3. **로그인 사용자 정보 바** (~55줄, 1908~1965) — `UserInfoBar` 컴포넌트 분리 가능 (관리자 토글, 로그아웃 등)

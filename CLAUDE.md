...
- 검증: `npm run verify:period` (직접 import #1~#22 + 소스 텍스트 가드 #G1~#G27).
  ⚠️ **미러 금지 — `src/utils.ts`를 직접 import**한다(그 파일은 import 0건이라 Node가 타입만 벗겨 실행).
  가드는 **선언이 아니라 사용부**를 단언하며 **변이 40종**(압축 행 재투입 · fail-safe 되돌림 ·
...
- **등락율도 기간을 따른다 — 차트·숫자·정렬·툴팁은 `viewByCode` 단일 소스 (⚠️ 회귀 주의)**:
  ...
  - **영속화 지점 0곳**: `viewByCode`·`rate`·`points`는 전부 매 렌더 파생값이고 `dailyMap`/`intradayMap`은

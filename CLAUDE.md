...
**가계부 재설계(2026-09-05 설계 완료, 구현 대기)** — 명세는 저장소 루트 `가계부_재설계_2026-09.md`.
거래 원장 레이어(단계 A) → 예산 진행·고정지출 확인·월 마감(B) → 계좌·카드 청구주기·할부·이체(C) →
대출 상환표·순자산(D) → 목표·가져오기·연도 정리(E) → 분석 확장(F). **한 세션에 한 단계**, 시작 프롬프트는 그 문서 §11,
구현 전 사용자 확인 결정 6건은 §9. 설계 세션에서는 코드를 바꾸지 않았다.

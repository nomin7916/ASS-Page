...
- **⚠️ 평가액을 더하는 모든 경로가 `depositRowEval(item, fxRate, isOverseas)` 하나를 공유**한다
  ...
  USD 표기 화면(자산검증 비교표·엑셀)은 짝 함수 `depositRowNative`(= USD + 원화 ÷ 환율)를 쓴다.
...
- **⚠️ 프레임별 환산 — 섞으면 그 화면만 조용히 틀린다**:
  ...
  · **USD 프레임**(개별 차트 TWR): `overseasUsdEvalAt(items, date, map, fxAt)`의 **선택 인자 `fxAt`**로
    원화를 USD 환산해 더하고, 흐름은 `App.tsx usdFlowRate`가 원화 행만 `1/환율`로 환산한다.
    **평가액과 흐름이 같은 환율(`usdFxAt`)을 써야** 원화 입금일의 ΔV와 흐름이 정확히 상쇄된다.
  · **fail-safe 2종(제거 금지)**: `externalFlowInRange`는 **`rateOf`가 없으면 원화 행을 집계하지
    않고**(호출부가 프레임을 모르므로), `overseasUsdEvalAt`는 **`fxAt`가 없으면 원화를 더하지 않는다**.
    둘 다 '미전달 시 반환값이 종전과 한 비트도 같다'는 하위호환의 축이기도 하다.
...
- **UI**: 예수금 행의 원화 칸은 **기존 라벨 셀(`colSpan`) 안**에 있다 — 새 `<td>`를 만들면 주식·펀드·
  예적금 행과 tfoot까지 전부 맞춰야 하고 한 곳만 놓쳐도 그 행부터 정렬이 깨진다(열 개수 불변).

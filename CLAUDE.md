...
- **추이 표 셀 구성**: 개별 계좌(`HistoryPanel`)는 열 3개뿐이라 평가자산 셀 = 금액 + `누적 ±N%`,
  전일대비 셀 = `%` + `±₩일간손익` + 입출금 배지로 **병기**한다. **일간 손익 ₩을 툴팁에서 셀로 승격**한
  이유는 "실제로 수익 내고 있나"에 답하는 건 %가 아니라 금액이기 때문. `dodAbsChange == null`(보류)은
  여전히 `'-'`(위 null 계약 준수).
  ⚠️ **통합 대시보드 추이표는 전일대비 셀에 입출금 배지를 렌더하지 않는다**(사용자 요청) — 그 표는
  `일간 손익`·`원금대비`·`투자원금` 전용 열을 이미 갖고 있어 배지가 중복 노이즈다. `netFlow` 필드
  자체는 헤더 '오늘 수익' 카드가 계속 쓰므로 **제거하지 말 것**(제거 시 "입금 ₩N 제외" 안내가 사라진다).

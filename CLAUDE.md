...
- **`CustomDatePicker` 확장 — 선택 인자 3종 + 결함 수정 5종 (사용처 6곳 공유, ⚠️ 회귀 주의)**:
  ...
  - **팝업을 `document.body`로 포털** — 그러지 않으면 z를 무엇으로 주든 조상이 만든 스태킹
    컨텍스트(`Z.dialog` 모달)에 갇혀 **z-1050 플로팅 창(계산기·관심종목·메모 달력)에 가려진다**.
    옛 네이티브 `<select>`는 브라우저 top layer라 항상 위였다.
...
- **게이트**(통과 못 하면 커밋 금지): 변경 영역의 `npm run verify:*`(calendar·tax·dividend·history·
  notice·twr·fx·brl·rebal-restore·transfer·overseas·flow·ladder·backtest·cal-detail·card-window·period·chart-sel·excel·compare **20종** 중 해당분) + `memory/tools/jsxcheck.mjs`

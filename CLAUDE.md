...
- **⚠️ 기준단가 우선순위 = `lofo` → `running` → `avgBuy`** (`resolveBasis`, #G3이 `indexOf` 순서로 단언).
  ...
  - **`trusted`** — 매입단가가 빈 매수 행은 로트를 만들지 못해 그 시점부터 풀 구성이 불완전해지므로
    `excludedSeen`으로 이후 행을 폴백시킨다(일자 없는 매수 행은 종전대로 **전 구간** 불신).
...
- **범위 밖(의도)**: 세율 적용 세액 · FIFO/후입선출 원가법 · 원가법 선택 토글(사용자가 LOFO 하나로
  ...
  않는다 — 이중 계상 혼선).
- 검증: `npm run verify:tax` (90건 — §9 `resolveAvgBuyPrice` · §10 `computeSellTaxRow` · §11
  `computeSellRealized` · **§13 `buildLofoLotSeries`**(사용자 실측 3건 재현 + 소급 방지·순차 소진·
  부족·신뢰도·부동소수 잔여) 참조 구현 미러 + **§12 렌더 배선 소스 텍스트 가드 #G1~#G13**).
  ...
  **추가**가 전부 통과한다. 변이 **총 25종**(LOFO 도입분 20종 포함)으로 검출을 실증했으므로, 가드를
  손볼 때 같은 변이가 여전히 잡히는지 다시 확인할 것.

...
- **표시 규약 = '지금 어느 열이 실제로 먹고 있는가'를 항상 보이게 한다**(3지점이 한 세트):
  ① **목표금액 모드**: 금액이 지정된 행의 **목표비중(%) 셀을 `opacity-40`**(포커스 시 원복) + 툴팁으로
  효력 정지 고지. 금액 미입력 행은 비중이 (힌트로) 그대로 먹으므로 **흐리게 하지 않는다**.
  ② **리밸런싱·적립식 모드**: 반대로 **목표금액 입력값을 `opacity-40`** + 헤더·tfoot을 회색으로 강등하고
  tfoot 부제를 `비중 기준 · 미적용`으로 바꾼다. 툴팁이 "투자선택을 목표금액으로 바꾸면 적용"을 안내한다.
  ③ **복원 모달의 `목표금액 우선` 배지도 `amountModeActive` prop으로 게이팅** — 리밸런싱 모드에서는

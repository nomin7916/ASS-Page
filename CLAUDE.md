...
- **장부액 관측(`bookDelta`) — 흡수 판정의 1순위 근거 (⚠️ 제거하지 말 것)**: `bookCostOf` =
  ...
  **공급자 3곳**: `HistoryPanel` `dailyMetricsByDate`, `useIntegratedData` `marketSeries.bookMap` →
  `computedIntHistory.bookTotal` → `intMonthlyHistory`, `buildHistoryCSV(..., bookByDate)`.

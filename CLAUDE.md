...
- **⚠️ 사용자 확정 규약 4종(2026-09) — 되돌리지 말 것**:
  ...
  정확한 값을 아는 척하지 않고 하한(=배당 과표)을 제시한다(적용하면 예상 과세가 정확히 0이 되어 실제와 일치).
  ③ **수량은 월 입금 내역 기준**(10,401주) — 실제 과세가 매겨진 대상이 곧 그 주식수다. 과표 계산 탭의
  `monthQty`(10,361주, 이벤트 누적)와 다를 수 있다.
  ④ **판정 임계 ₩1** — 실제 과세금은 사용자가 직접 입력하는 값이라 자주 바뀌지 않아 노이즈가 적다.
- **저장 = `taxBaseHistory[code].avgTaxBaseAdj[ym] = { value, kind, exTaxBase, taxAmount, qty, exDate, prevAvg, at }`**.
  ⚠️ **`avgTaxBase`(수동 입력)와 합치지 말 것** — 합치면 "사용자가 직접 친 값"과 "실제 과세로 역산한 값"을
  구분할 수 없어 출처 툴팁을 띄울 수 없고, 사용자가 넣어 둔 값을 조정이 조용히 덮는다. 우선순위는
  **수동 입력 > 앵커 반영 자동값 > 순수 자동값**. 산출 근거(exDate·taxAmount·qty)를 함께 박제하는 이유는
  나중에 배당락일·분배금 입력이 바뀌어도 확정한 앵커가 조용히 흔들리지 않게 하기 위해서다.
- **⚠️ 영속화 배선 2곳이 **둘 다** 화이트리스트 손나열이다 — 하나만 빠져도 조용히 유실**:
  ① `usePortfolioState._ensureTaxBase`(**재구축기** — 빠지면 배당 과표를 한 글자만 고쳐도 조정이 통째로
  삭제된다) ② `App.tsx portfolioStructureKey.taxBaseKey`(빠지면 `portfolioUpdatedAt`이 안 올라 STATE 저장이
  스킵된다 — `historyVerifyKey`·`targetAmount`와 동일 버그 클래스). 그 외 영속화 신규 지점 **0곳**.
- **⚠️ `computeRunningAvgSnapshots(events, anchors?)`의 `anchors`는 선택 인자 — 미전달 시 반환값이
  종전과 한 바이트도 다르지 않다**(하위호환의 축). 앵커는 **같은 날짜 매매보다 먼저** 적용한다
  (배당락일 매수는 그 배당 권리가 없고, 이 순서라야 반환 배열의 날짜 오름차순이 유지되어 호출부의
  `else break` 스캔이 성립한다). 마지막 이벤트 **이후** 날짜의 앵커도 반드시 흘려보낼 것(실데이터의
  기본 경로 — 배당락일은 보통 마지막 매매보다 뒤에 온다).
- **⚠️ 앵커는 3소비자에 **모두** 전달**: `computeMonthlyAvgForGrid`(그리드 자동값) ·
  `buildSortedEventsWithAvg`(계산기 러닝 평균) · `resolveAvgAdjState`(월 입금 내역). 하나라도 빠지면
  같은 달에 두 화면이 다른 평균 과표를 보여준다. 앵커 생성은 **`buildAvgTaxAnchors` 공유 함수**로만
  (손복제 금지). ⚠️ 그 함수의 exDate 폴백은 truthy가 아니라 **ISO 형식**으로 판정한다 — 손상된
  `exDate`('bad' 등)는 truthy라 `|| monthEndOfYm(ym)`을 건너뛰고 필터에서 앵커가 통째로 사라진다
  (사용자가 확정한 조정이 조용히 무시된다. 검증이 실제로 잡은 결함).
- **⚠️ null 계약** — `taxAmount == null`(미입력)과 `0`(과세 없음)은 **다른 사건**이다. `safeNum(null)`이
  ...
  배당 과표 없음·수량 0·음수 과세·역산 결과 0 이하도 전부 `null`(모르는 값을 단언하지 않는다).
- **⚠️ 제안 판정의 예상 과세는 화면의 `expected`(monthQty)가 아니라 **관측 수량 기준**이다** — 화면
  기준으로 재면 조정을 적용해 평균 과표가 정확해진 뒤에도 수량 차이 때문에 제안이 **영원히 꺼지지 않는다**.
- **UI**: 월 입금 내역 셀 = **표시만**(`⚑ 과표 9,946.72` / 적용 후 `⚑ 조정 …`), 적용은 **과표 계산 탭**에서
  (사용자 확정). 툴팁 문구는 **`avgAdjTooltip` 공유 포매터**로만 만든다(손복제하면 같은 조정을 두 화면이
  다르게 설명하고 한쪽만 고쳐진 채 남는다 — `periodNoun` 규약과 동일).
- **⚠️ 한 지급월에 2건이 합산된 셀이어도 조정은 지배 소스의 `exYm` 하나만 본다** — 저장 키가 전부
  배당락월이라 합산과 무관하게 정확하다(툴팁이 그 사실을 밝힌다).
- **별도 창**: `dividendCall` byId 화이트리스트에 `updateTaxBaseAvgAdj` 필수(빠지면 카드 창에서만 조정이
  안 되는데 오류도 안 난다). prop 체인 4-hop(App → DividendSummaryTable → KrEtfTaxMatrix / CardWindow).
- **범위 밖(의도)**: 월 입금 내역에서의 직접 적용 · 조정 이력(여러 시점 앵커의 타임라인 UI) ·
  해외 계좌(한국 ETF 과표 전용) · 조정값의 자동 적용(사용자가 버튼으로 명시 적용).
- 검증: `npm run verify:tax` (118건 — §14 참조 미러 `#160~#172` + §15 소스 텍스트 가드 `#G15~#G25`).
  ⚠️ 가드는 **선언이 아니라 사용부**를 단언하며 **변이 15종**(화이트리스트 2곳 각각 제거 · 별도 창 등록 제거 ·
  앵커 미전달 2곳 · 셀 게이트 삭제 2곳 · 공유 함수를 손계산으로 · 미러 드리프트 · 앵커 순서 뒤집기 ·
  미입력을 0원으로 · 임계 확대 · 수동 수량 무시 · exDate 폴백 제거 · 툴팁 손복제)으로 **실제 검출을 확인**했다.
  `src/krEtfTaxHelpers.ts`의 `solveAvgTaxBaseFromTax`·`buildAvgTaxAnchors`·`avgTaxBaseAdjNeeded` 본문과
  `scripts/verify-kretf-tax.mjs`의 미러를 **항상 1:1 동기화**할 것(#G24·#G25가 문자 단위로 대조한다).

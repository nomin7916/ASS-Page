...
1. **시점별 신뢰도** — 전역 로 러닝 평균 신뢰도를 재던 탓에 매도보다 **뒤에 있는**
   불완전 매수 행 1건이 그 매도의 완전한 평균을 폐기시켜 현재 시점 값으로 폴백했다(실측 +50,000 →
   **−83,333**). 폴백이면 라 ⚠도 구조적으로 못 뜨고, '행 추가' 후 수량만
   입력한 상태로도 발동했다 → 행 시점까지의 으로 판정.
2. **같은 날짜 정렬 결정성** — 만으로는 비교값 0이라 배열 삽입 순서에 부호가 의존했다
   (기준 15,000이면 −150,000, 10,000이면 +100,000) → (매수 우선) 공유 비교자.
3. **과세 3열 각주 4줄 복원** — 실현손익 각주를 끼워 넣다 과세 금액 정의·min 규약·평균단가 폴백
   3단계·① 현재 시점 값 경고가 통째로 삭제돼 있었다.
4. **요약 바 래퍼 게이트** — 내부 매도 줄보다 좁아 '합계 제외 N건' 진단이 도달 불가였다.
5. **'실현손익 미산출' → '합계 제외'** — 값이 뜬 행을 미산출이라 부르던 라벨 정정.

가드도 강화했다. 리뷰가 실증했듯 **'존재' 단언만으로는**  두 줄 맞바꾸기·매도 분기
 추가가 71/71로 통과한다 → 우선순위는  순서 비교, 금지 코드는 분기 본문을
잘라 **부재**로, 게이트는 **래퍼⊇내부 포함관계**로 단언한다.
**변이 20종(1차 11 + 2차 9) 전부 검출** — 2차 N4가 처음엔 통과해 #G3을 한 번 더 강화했다.
